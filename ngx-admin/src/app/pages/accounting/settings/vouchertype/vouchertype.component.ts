...
import { VouchertypeService } from "app/services/vouchertype.service";
...
export class VouchertypeComponent implements OnInit {
  ...
  public pageNavigation = "Voucher Type";
  ...
  private getVoucherType() {
    this.vouchertypeService.getVoucherType().subscribe((returns: any) => {
      if (returns.success) {
  ...
  onGridReady(params) {
    ...
    this.vouchertypeService.getVoucherType().subscribe((data: any) => {
      if (data.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.selectedRow = event.node.data;
      var voucherTypeId = event.node.data.voucherTypeId;
  ...
  private agDelete(event) {
    ...
    if (result) {
      this.master.voucherTypeId = event.node.data.voucherTypeId;
      this.vouchertypeService
        .deleteVoucherType(this.master)
        .subscribe((returns: any) => {
          if (returns.success) {
