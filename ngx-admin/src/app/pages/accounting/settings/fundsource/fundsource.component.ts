...
import { FundsourceService } from "app/services/fundsource.service";
...
@Component({
  selector: "ngx-fundsource",
  templateUrl: "./fundsource.component.html",
  styleUrls: ["./fundsource.component.scss"],
})
export class FundsourceComponent implements OnInit {
  ...
  public pageNavigation = "Fund Source";
  ...
  public getDuplicate() {
    //debugger;
    this.fundsourceService.getDuplicateFundSource(this.master.fundSourceId, this.master.fundSourceName).subscribe((returns: any) => {
      //debugger;
  ...
  private save() {
    ...
    if (this.master.fundSourceName == '' || this.master.fundSourceName == null) {
      this.toastrService.danger("Please insert fund source name", "Message");
      ...
      this.toastrService.danger("Duplicate fund source name", "Message");
      this.commonService.valueSet("create");
      return false;
    }

    this.fundsourceService.saveFundSource(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
        ...
        this.show = true;
        //////////////Grid Refresh ///////////////////
        this.fundsourceService.getFundSource().subscribe((data: any) => {
          if (data.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.selectedRow = event.node.data;
      var fundSourceId = event.node.data.fundSourceId;

      this.fundsourceService
        .getFundSourceById(fundSourceId)
        .subscribe((data: any) => {
          if (data.success) {
            //debugger;
  ...
  private agDelete(event) {
    ...
    if (result) {
      //debugger;
      this.master.fundSourceId = event.node.data.fundSourceId;
      this.fundsourceService
        .deleteFundSource(this.master)
