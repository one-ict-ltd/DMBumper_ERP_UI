...
@Component({
  selector: 'ngx-paymentvoucher-v2',
  templateUrl: './paymentvoucher-v2.component.html',
  styleUrls: ['./paymentvoucher-v2.component.scss']
})
export class PaymentvoucherV2Component implements OnInit {
  master: {
  ...
  public pageNavigation = "Payment Voucher";
  ...
  public ButtonAction() {
    if (this.commonService.buttonClicked == "create") {
      ...
      //console.log(this.master.companyId)
      this.show = false;
      this.disabled = false;
  ...
  public agButtonAction() {
    ...
    if (this.commonService.agButtonClicked == "refresh") {
  ...
  public getDropdownData() {
    ...
    this.comboService.getCompany().subscribe((returns: any) => {
      if (returns.status) {
        ...
        this.master.companyId = returns.data[0].companyId;
  ...
  public getLedgerOBC() {
    ...
    this.master.ledgerOBCBalance = "0";
    this.master.partyBalance = "0";
    this.comboService.getLedgersForVoucher(this.master.companyId, this.master.sbuId).subscribe((returns: any) => {
      //console.log(returns);
      let res = null;

      // res = returns.data.filter((it) => it.ledgerTypeId != 1 && it.ledgerTypeId != 2);
      res = returns.data;
  ...
  public getVoucher() {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      this.voucherService.getVoucherWithDate(0, 2, this.commonService.DateFormat(this.master.loadFromDateShow), this.commonService.DateFormat(this.master.loadToDateShow)).subscribe((data: any) => {
        ////debugger;
        if (data.success) {
  ...
  private save() {
    console.log('lstcostmodel ', this.master.lstcostmodel);
    ...
    for (let i = 0; i < this.master.lstdetailmodel.length; i++) {
      ...
      if (totalCost > 0) {
        if (this.master.lstdetailmodel[i].amount != totalCost) {
    ...
    //debugger;
    if (this.master.companyId == 0) {
      this.toastrService.danger("Please Select a Company name", "Message");
      this.commonService.valueSet("create");
      return false;
    }
  ...
  public LoadCostCentreByCategoryIdandLocationId() {
    ...
    this.costcentreService.getCostCentrebyCategoryIdandLocationId(costCentreLocationId, costCentreCategoryId).subscribe((returns: any) => {
      ...
      console.log("costCentres: ", this.costCentres)
    });
  }
  ...
  public addDetails_BAK2024Dec12(dialog: TemplateRef<any>) {
    ////debugger
  ...
  public deleteDetail(index: any) {
    ...
    let ledgerId = this.selectedRow.ledgerId;
    let partyId = this.selectedRow.partyId;

    this.master.lstcostmodel = this.master.lstcostmodel.filter((item) => item.ledgerId != ledgerId && item.partyId != partyId);

    this.master.lstcostfinalmodel = this.master.lstcostfinalmodel.filter((item) => item.ledgerId != ledgerId && item.partyId != partyId);
  ...
  public editCost(index: any) {
    ...
    this.master.costAmountBalance = this.commonService.roundWithDecimalPoint((
      this.master.costforAmount - totalamount + this.selectedRow.amount), 2);
    this.calculateBalance();
  }
  ...
  public getCostCentreCategory() {
    this.costcentreService.getCostCentreCategory().subscribe((retuns: any) => {
      if (retuns.success) {
        ...
        console.log('CostCentreCategoryList:', this.CostCentreCategoryList);
      }
    })
  }
