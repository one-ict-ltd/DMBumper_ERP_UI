...
@Component({
  selector: "ngx-rpt-trialbalance",
  templateUrl: "./rpt-trialbalance.component.html",
  styleUrls: ["./rpt-trialbalance.component.scss"],
})
export class RptTrialbalanceComponent implements OnInit {
  ...
  private getDropdownData() {
    ...
    let rptTypeList = [
      { value: "GroupWise", text: "Group Wise" },
      { value: "LedgerWise", text: "Ledger Wise" },
      { value: "GroupWiseBalance", text: "Group Wise Balance Details" },
      { value: "LedgerWiseBalance", text: "Ledger Wise Balance Details" },
    ]
    this.reportTypeList = rptTypeList.map((val) => ({
      id: val.value,
      name: val.text,
    }));
  }
  ...
  tdrAmount: any = "-";
  tcrAmount: any = "-";
  tcurrentDrAmount: any = "-";
  tcurrentCrAmount: any = "-";

  showDetails: boolean = false;
  showReport() {
    if (this.reportType == "LedgerWise" || this.reportType == "GroupWise") {
      this.showbody = true;
      this.showDetails = false;

      this.tdrAmount = this.currencyFormatter(this.bodyData.reduce((ttl, item) => ttl + (item.isSum === 1 ? item.drAmount : 0), 0));
      this.tcrAmount = this.currencyFormatter(this.bodyData.reduce((ttl, item) => ttl + (item.isSum === 1 ? item.crAmount : 0), 0));
    }
    else {
      this.showbody = false;
      ...
      this.tobDrAmount = this.currencyFormatter(this.bodyData.reduce((ttl, item) => ttl + (item.isSum === 1 ? item.obDrAmount : 0), 0));
      this.tobCrAmount = this.currencyFormatter(this.bodyData.reduce((ttl, item) => ttl + (item.isSum === 1 ? item.obCrAmount : 0), 0));

      this.tcurrentDrAmount = this.currencyFormatter(this.bodyData.reduce((ttl, item) => ttl + (item.isSum === 1 ? item.currentDrAmount : 0), 0));
      this.tcurrentCrAmount = this.currencyFormatter(this.bodyData.reduce((ttl, item) => ttl + (item.isSum === 1 ? item.currentCrAmount : 0), 0));
    }
  }
  ...
  private getReportData() {
    ...
    */
    this.apiUrl = `AccountsReport/GetTrialBalanceReport?companyId=${this.ddlSelected.id
      }&sbuId=${this.branchSelected.id}&accountGroupId=0&fromDate=${this.commonService.DateFormat(this.fromdateSelected
      )}&toDate=${this.commonService.DateFormat(this.todateSelected)}&rptType=${this.reportType}&reportFormat=pdf`;
  ...
  private onPreview() {
    if (this.reportType == "") {
      this.toastrService.warning("Please select a report type.", "Validate")
      return;
    }
  ...
  base64Pdf: any;
  private getCrReport(reportFormat: any = 'pdf') {//"AccountsReport/GetTrialBalanceReport?"

    //this.apiUrl = `AccountsReport/GetTrialBalanceReport?companyId=${this.ddlSelected.id}&sbuId=${this.branchSelected.id}&accountGroupId=0&fromDate=${this.commonService.DateFormat(this.fromdateSelected)}&toDate=${this.commonService.DateFormat(this.todateSelected)}&rptType=${this.reportType}&reportFormat=${reportFormat}`;

    this.apiUrl = `AccountsReport/GetTrialBalanceReport?companyId=${this.ddlSelected.id
      }&sbuId=${this.branchSelected.id}&accountGroupId=0&fromDate=${this.commonService.DateFormat(this.fromdateSelected
      )}&toDate=${this.commonService.DateFormat(this.todateSelected)}&rptType=${this.reportType}&reportFormat=${reportFormat}`;
  ...
  currencyFormatter(currency: number = 0) {
    return this.commonService.currencyFormatterAccStandard(currency);
  }
  ...
  private getReportDataExcel() {
    this.apiUrl = `AccountReport/getRptTrialBalance?companyId=${this.ddlSelected.id}&sbuId=${this.branchSelected.id}&accountGroupId=0&fromDate=${this.fromdateSelected.toString().substring(3, 15)}&toDate=${this.todateSelected
      .toString().substring(3, 15)}&rptType=${this.reportType}`;
