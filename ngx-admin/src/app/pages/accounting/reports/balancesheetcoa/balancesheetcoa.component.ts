...
@Component({
  selector: 'ngx-balancesheetcoa',
  templateUrl: './balancesheetcoa.component.html',
  styleUrls: ['./balancesheetcoa.component.scss']
})
export class BalancesheetcoaComponent implements OnInit {
  ...
  private getReportData() {
    if (this.ApprovalStatusSelected == null || this.ApprovalStatusSelected == undefined) {
      this.toastrService.danger("Pleae Select Type", "Message");
      return;
    }
    this.apiUrl = `AccountReport/getRptBalanceSheetCoa?companyId=${this.companySelected.id
      }&sbuId=${this.branchSelected.id
      }&fromDate=${this.fromdateSelected
        .toString()
        .substring(3, 15)}&toDate=${this.todateSelected
          .toString()
          .substring(3, 15)}&level=${this.ApprovalStatusSelected['id']}`;
    this.commonService.getReportData(this.apiUrl).subscribe((returns: any) => {
      if (returns.success) {
        this.bodyDataNonCurAsset = returns.data;
  ...
  public generateReport(buttonAction: any) {
    ...
    // }
    this.getCrReport(buttonAction);
  }

  private getCrReport(reportFormat: any = 'pdf') {//"AccountsReport/GetTrialBalanceReport?"

    this.apiUrl = `AccountsReport/GetBalanceSheetfromCOAReport?companyId=${this.companySelected.id
      }&sbuId=${this.branchSelected.id
      }&fromDate=${this.commonService.DateFormat(this.fromdateSelected
      )}&toDate=${this.commonService.DateFormat(this.todateSelected)}&level=${this.ApprovalStatusSelected['id']}&reportFormat=${reportFormat}`;
