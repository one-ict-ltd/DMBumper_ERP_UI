...
@Component({
  selector: "ngx-rpt-paymentreceivenew",
  templateUrl: "./rpt-paymentreceivenew.component.html",
  styleUrls: ["./rpt-paymentreceivenew.component.scss"],
})
export class RptPaymentreceivenewComponent implements OnInit {
  ...
  public pageNavigation = "Receipt Payment Book";
  public tableHeader = ["Account Code", "Account Name", "Amount"];
  ...
  private getReportData() {
    this.apiUrl = `AccountReport/getRptPaymentReceivedNew?companyId=${this.companySelected.id
      }&sbuId=${this.branchSelected.id}&ledgerId=0&fromDate=${this.fromdateSelected.toString().substring(3, 15)}&toDate=${this.todateSelected.toString().substring(3, 15)}`;
    this.commonService.getReportData(this.apiUrl).subscribe((returns: any) => {
      if (returns.success) {

        this.bodyDataReceived = returns.data.filter(
          (item) =>
            item.voucherTypeName === "OB" || item.voucherTypeName === "RECEIPT" || item.voucherTypeName === "CONTRARECEIPT"
        );

        this.bodyDataPayment = returns.data.filter(
          (item) =>
            item.voucherTypeName === "CB" || item.voucherTypeName === "PAYMENT" || item.voucherTypeName === "CONTRAPAYMENT"
        );
  ...
  private onExportCSV() {
    this.GetRptPaymentReceiptExcel();
  }
  ...
  public generateReport(buttonAction: any) {
    ...
    // this.generatePdfBankBook(buttonAction, fileName, content);
    this.GetRptPaymentReceipt();
  }
  ...
  public GetRptPaymentReceipt() {
    this.accountreportService.RptPaymentReceipt(this.companySelected.id, this.branchSelected.id, 0, this.fromdateSelected.toString().substring(3, 15), this.todateSelected.toString().substring(3, 15), "Pdf").subscribe((returns: any) => {
      this.commonService.GenerateBase64ToReport(returns);
    });
  }

  public GetRptPaymentReceiptExcel() {
    this.accountreportService.RptPaymentReceipt(this.companySelected.id, this.branchSelected.id, 0, this.fromdateSelected.toString().substring(3, 15), this.todateSelected.toString().substring(3, 15), "Excel").subscribe((returns: any) => {
      this.commonService.GenerateBase64ToReport(returns);
    });
  }
  ...
  public generatePdfVoucherReport(buttonAction: any, fileName: string, content: any, datalength: number) {
