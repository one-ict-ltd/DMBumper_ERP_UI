...
@Component({
  selector: "ngx-rpt-balancesheet",
  templateUrl: "./rpt-balancesheet.component.html",
  styleUrls: ["./rpt-balancesheet.component.scss"],
})
export class RptBalancesheetComponent implements OnInit {
  ...
  public pageNavigation = "Balance Sheet";
  public tableHeader = [
    "Particulars",
    this.yearName + " (Tk.)",
    "Previous Year (Tk.)",
  ...
  private getReportData() {
    //debugger;
    this.apiUrl = `AccountReport/getRptBalanceSheet?companyId=${this.companySelected.id
    ...
    this.commonService.getReportData(this.apiUrl).subscribe((returns: any) => {
      if (returns.success) {
        ...
        this.bodyData.forEach(
          (a) => (this.TotalAsset += parseFloat(a.currentAmount))
        );
        ...
        this.bodyDataExpense.forEach(
          (a) => (this.TotalLiability += parseFloat(a.currentAmount))
        );
  ...
  private getReportDataProfit() {
    ...
    this.commonService
      ...
      .subscribe((returnss: any) => {
        if (returnss.success) {
          this.bodyDataProfitLoss = returnss.data.filter(
            (item) => item.natureName === "Incomes"
          );
          this.TotalIncome = 0;
          this.bodyDataProfitLoss.forEach(
    ...
    doc.html(content, {
      callback: function (doc) {
        ...
        autoTable(doc, {
          ...
          columnStyles: {
            0: { halign: "center", minCellWidth: 100 },
            1: { halign: "right", minCellWidth: 200 },
            2: { halign: "right", minCellWidth: 200 },
          },
