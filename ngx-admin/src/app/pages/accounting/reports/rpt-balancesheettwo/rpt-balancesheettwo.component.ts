...
@Component({
  selector: "ngx-rpt-balancesheettwo",
  templateUrl: "./rpt-balancesheettwo.component.html",
  styleUrls: ["./rpt-balancesheettwo.component.scss"],
})
export class RptBalancesheettwoComponent implements OnInit {
  ...
  public bodyDataAsset: any = [];
  public bodyDataLiability: any = [];
  ...
  public TotalLiabilityPrevYear = 0;
  ...
  private getReportData() {
    //debugger;
    this.apiUrl = `AccountReport/getRptBalanceSheetTwo?companyId=${this.companySelected.id
    ...
    this.commonService.getReportData(this.apiUrl).subscribe((returns: any) => {
      if (returns.success) {
        this.bodyDataAsset = returns.data.filter(
          (item) => item.natureName === "Assets"
        );
        this.TotalAsset = 0;
        this.bodyDataAsset.forEach(
          (a) => (this.TotalAsset += parseFloat(a.totalCurrentAmount))
        );
        this.TotalAssetPrevYear = 0;
        this.bodyDataAsset.forEach(
          (a) => (this.TotalAssetPrevYear += parseFloat(a.previousAmount))
        );

        this.bodyDataLiability = returns.data.filter(
          (item) => item.natureName === "Liabilities"
        );
        this.TotalLiability = 0;
        this.bodyDataLiability.forEach(
          (a) => (this.TotalLiability += parseFloat(a.totalCurrentAmount))
        );
        this.TotalLiabilityPrevYear = 0;
        this.bodyDataLiability.forEach(
          (a) => (this.TotalLiabilityPrevYear += parseFloat(a.previousAmount))
        );
      } else {
  ...
  private getReportDataProfit() {
    this.apiUrl2 = `AccountReport/getRptIncomeStatement?companyId=${this.companySelected.id
      }&sbuId=${this.branchSelected.id}&fromDate=${this.fromdateSelected
        .toString()
        .substring(3, 15)}&toDate=${this.todateSelected
          .toString()
    ...
    this.TotalLOE = this.TotalLiability + this.NetProfit;
    this.TotalLOEPrevYear =
      this.TotalLiabilityPrevYear + this.NetProfitPrevYear;
  }
