...
@Component({
  selector: "ngx-rpt-incomestmentgrossformat",
  templateUrl: "./rpt-incomestmentgrossformat.component.html",
  styleUrls: ["./rpt-incomestmentgrossformat.component.scss"],
})
export class RptIncomestmentgrossformatComponent implements OnInit {
  ...
  public bodyDataOtherNonIncome: any = [];
  ...
  public TotalSales = 0;
  public TotalCostofSale = 0;
  public GrossProfit = 0;
  public TotalOtherNonIncome = 0;
  ...
  private getReportData() {
    //debugger;
    this.apiUrl = `AccountReport/getRptIncomeStatementGrossFormat?companyId=${this.companySelected.id
    ...
    this.commonService.getReportData(this.apiUrl).subscribe((returns: any) => {
      if (returns.success) {
        this.bodyData = returns.data.filter(
          (item) => item.noteName === "Sales"
        );
        this.TotalSales = 0;
        this.bodyData.forEach(
          (a) => (this.TotalSales += parseFloat(a.currentAmount))
        );

        this.bodyDataCostofSale = returns.data.filter(
          (item) => item.noteName === "Cost of Sales"
        );
        this.TotalCostofSale = 0;
        this.bodyDataCostofSale.forEach(
          (a) => (this.TotalCostofSale += parseFloat(a.currentAmount))
        );

        this.bodyDataOtherNonIncome = returns.data.filter(
          (item) => item.noteName === "Others/Non-Operating Income"
        );
        this.TotalOtherNonIncome = 0;
        this.bodyDataOtherNonIncome.forEach(
          (a) => (this.TotalOtherNonIncome += parseFloat(a.currentAmount))
        );

        this.bodyDataExpense = returns.data.filter(
          (item) => item.noteName === "Expenses"
        );
        this.TotalExpense = 0;
      ...
      this.GrossProfit = this.TotalSales - this.TotalCostofSale;

      this.TotalIncome =
        this.TotalSales + this.TotalOtherNonIncome - this.TotalCostofSale;

      this.NetProfit =
        this.TotalSales - (this.TotalExpense + this.TotalCostofSale);
    });
  }
