...
@Component({
  selector: "ngx-rpt-bankbook",
  templateUrl: "./rpt-bankbook.component.html",
  styleUrls: ["./rpt-bankbook.component.scss"],
})
export class RptBankbookComponent implements OnInit {
  public date = new Date().getFullYear();
  public fromdateSelected = new Date();
  public todateSelected = new Date();

  public pageNavigation = "Bank Book";
  public tableHeader = [
    "Date",
    "Particulars",
    "Voucher No",
  ...
  public getLedgerIdData() {
    this.comboService.getLedgersForNoteDetails(this.companySelected.id, this.branchSelected.id, 0, 0, 2)
  ...
  private getReportData() {
    ...
    this.apiUrl = `AccountReport/getRptBankBook?companyId=${this.companySelected.id
      }&sbuId=${this.branchSelected.id
      }&ledgerId=${ledgerId}&fromDate=${this.fromdateSelected
        .toString()
    ...
    this.commonService.getReportData(this.apiUrl).subscribe((returns: any) => {
      if (returns.success) {
        ...
        this.bodyDataReceived = returns.data.filter(
          // (item) => (item.voucherTypeName === "OB" || item.voucherTypeName === "RECEIPT" || item.voucherTypeName === "CONTRA" || item.voucherTypeName === "JOURNAL") && item.drAmount != 0
        ...
        this.bodyDataTotalReceived = returns.data.filter(
          // (item) => (item.voucherTypeName === "RECEIPT" || item.voucherTypeName === "CONTRA" || item.voucherTypeName === "JOURNAL") && item.drAmount != 0
        ...
        this.bodyDataPayment = returns.data.filter(
          // (item) => (item.voucherTypeName === "PAYMENT" || item.voucherTypeName === "CONTRA" || item.voucherTypeName === "JOURNAL") && item.crAmount != 0
        ...
        // this.bodyDataClosing = returns.data.filter(
        //   (item) => (item.voucherTypeName === "PAYMENT" || item.voucherTypeName === "CONTRA" || item.voucherTypeName === "JOURNAL") && item.crAmount != 0
        // );
        // this.TotalPayment = 0;
        // this.bodyDataClosing.forEach(
        //   (a) => (this.TotalPayment += parseFloat(a.crAmount))
        // );
