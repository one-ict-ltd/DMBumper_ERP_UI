...
export class FiscalyearComponent implements OnInit {
  ...
  master: {
    ...
    yearStartDateShow: Date;
    yearStartDate: string;
    yearEndDateShow: Date;
    yearEndDate: string;
    lockDateShow: Date;
    lockDate: string;
    isActive: number;
  ...
  public pageNavigation = "Fiscal Year";
  ...
  public getMaster() {
    this.master = {
      ...
      yearName: "",
      financialYearName: "",
      yearStartDateShow: new Date(),
      yearStartDate: new Date().toLocaleDateString() + " " + new Date().toLocaleTimeString(),
      yearEndDateShow: new Date(),
      yearEndDate: new Date().toLocaleDateString() + " " + new Date().toLocaleTimeString(),
      lockDateShow: new Date(),
      lockDate: new Date().toLocaleDateString() + " " + new Date().toLocaleTimeString(),
      isActive: 1,
      islocked: 1,
  ...
  public getDuplicate() {
    this.fiscalyearService.getDuplicateFiscalYear(this.master.fiscalYearId, this.master.yearName)
  ...
  private save() {
    ...
    if (this.master.companyId == 0 || this.master.companyId == null) {
      ...
      this.commonService.valueSet("create");
      return false;
    } else if (this.master.yearName == '' || this.master.yearName == null) {
      this.toastrService.danger("Please insert year name", "Message");
      this.commonService.valueSet("create");
      return false;
    } else if (this.master.countData != 0) {
      this.toastrService.danger("Duplicate year name", "Message");
      this.commonService.valueSet("create");
      return false;
    }

    this.fiscalyearService.saveFiscalYear(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
  ...
  public getActualStartDate(event: any) {
    var dateCon = event.toLocaleDateString() + " " + event.toLocaleTimeString();
    if (dateCon != "") {
      this.master.yearStartDate = dateCon;
    }
  }
  public getActualEndDate(event: any) {
    var dateCon = event.toLocaleDateString() + " " + event.toLocaleTimeString();
    if (dateCon != "") {
      this.master.yearEndDate = dateCon;
    }
  }
  public getActualLockDate(event: any) {
    var dateCon = event.toLocaleDateString() + " " + event.toLocaleTimeString();
    if (dateCon != "") {
      this.master.lockDate = dateCon;
    }
  }
