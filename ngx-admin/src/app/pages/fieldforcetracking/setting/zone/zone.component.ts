...
export class ZoneComponent implements OnInit {
  ...
  private save() {
    if (this.master.Name == "") {
      ...
      this.fieldforcemasterService.saveZone(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.fieldforcemasterService.getZone(0).subscribe((data: any) => {
            if (data.length > 0) {
              this.rowData = data;
            }
          });
  ...
  private agDelete(event) {
    ...
    this.fieldforcemasterService.deletezone(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.fieldforcemasterService.getZone(this.master.Id).subscribe((data: any) => {
        if (data.length > 0) {
          this.master = data[0];
        }
      });
