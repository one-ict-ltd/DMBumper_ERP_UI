...
export class MarketComponent implements OnInit {
  ...
  private save() {
    if (this.master.Name == "") {
      ...
      this.fieldforcemasterService.saveTerritory(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.fieldforcemasterService.getMarket(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    this.master.Id = event.node.data.MarketId;
    this.fieldforcemasterService.deleteMarket(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.fieldforcemasterService.getMarket(this.master.Id).subscribe((data: any) => {
        if (data.success) {
  ...
  public GetTerritory(RegionCode) {
    this.master.TerritoryCodeSelected = {};
    this.fieldforcemasterService.getTerritorybyAreacode(RegionCode).subscribe((retuns: any) => {
      if (retuns.success) {
