...
export class RegionComponent implements OnInit {
  ...
  private save() {
    if (this.master.Name == "") {
      this.toastrService.danger("Region name is required", "Message");
      this.commonService.valueSet("create");
    }
    else {
      this.show = true;
      var button = this.commonService.buttonClicked;
      this.fieldforcemasterService.saveRegion(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.fieldforcemasterService.getRegion(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    ...
    this.fieldforcemasterService.deleteRegion(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.fieldforcemasterService.getRegion(this.master.Id).subscribe((data: any) => {
        if (data.success) {
