...
@Component({
  selector: 'ngx-employee-attendance-report-for-ffm',
  templateUrl: './employee-attendance-report-for-ffm.component.html',
  styleUrls: ['./employee-attendance-report-for-ffm.component.scss']
})
export class EmployeeAttendanceReportForFFMComponent implements OnInit {
  ...
  pageNavigation = "Monthly Employee Attendance Report For FFM";
  ...
  generateCrReport(reportFormat: any) {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      ...
      this.apiUrl = `AttendanceReport/GetMonthlyEmployeeAttendanceReportForFFM?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&zoneCode=${this.zoneCode}&regionCode=${this.regionCode}&areaCode=${this.areaCode}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&empCode=${this.mioCode}`;
  ...
  private getReportData() {
    ...
    let userInfo = this.commonService.GetUserProfileJson();
    this.apiUrl = `AttendanceReport/GetMonthlyEmployeeAttendanceReportForFFM?reportFormat=Pdf&userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&zoneCode=${this.zoneCode}&regionCode=${this.regionCode}&areaCode=${this.areaCode}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&empCode=${this.mioCode}`;
    debugger;
    this.commonService.GetCrystalReportData(this.apiUrl).subscribe((returns: any) => {
      let res = JSON.parse(returns);
      if (res.status) {
