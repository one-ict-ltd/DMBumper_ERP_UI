...
@Component({
  selector: "ngx-chemistwisevisit",
  templateUrl: "./chemistwisevisit.component.html",
  styleUrls: ["./chemistwisevisit.component.scss"],
})
export class ChemistwisevisitComponent implements OnInit {
  public pageNavigation = "Customer Wise Visit Report";
  public tableHeader = [
    "#",
    "MIO Name",
    "Mobile",
  ...
  private getReportData() {
    ...
    //this.bodyData = [];
    var ChemistId = 0;
    if (this.master.CustomerSelected != null) {
      ChemistId = this.master.CustomerSelected["id"];
    }

    this.apiUrl = `Report/getChemistWiseVisitReport?ZoneId=${this.master.ZoneId
      ...
      }&ChemistId=${ChemistId}&fromDate=${this.master.fromDate
        .toDateString()
    ...
    this.commonService.getReportData(this.apiUrl).subscribe((returns: any) => {
      if (returns.success) {
        ...
        this.bodyData.forEach(
          (a) => (this.TotalInvoice += parseFloat(a.invoiceAmount))
        );
  ...
  public CustomerList = [];
  public GetCustomerbyMarket(MarketName) {
    this.master.CustomerSelected = {};
    this.miovisitService.getCustomer(MarketName).subscribe((retuns: any) => {
      if (retuns.success) {
        this.CustomerList = retuns.data.map((val: any) => ({
          id: val.chemistid,
          name: val.chemistname,
        }));
      }
    });
  }
