...
@Component({
  selector: 'ngx-digital-gift-disbursement-report',
  templateUrl: './digital-gift-disbursement-report.component.html',
  styleUrls: ['./digital-gift-disbursement-report.component.scss']
})
export class DigitalGiftDisbursementReportComponent implements OnInit {
  ...
  pageNavigation = "Digital Gift Disbursement Report";
  ...
  generateCrReport(reportFormat: any) {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      ...
      this.apiUrl = `AttendanceReport/GetDigitalGiftDisbursementReport?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&zoneCode=${this.zoneCode}&regionCode=${this.regionCode}&areaCode=${this.areaCode}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&empCode=${this.mioCode}`;
  ...
  private getReportData() {
    ...
    let userInfo = this.commonService.GetUserProfileJson();
    this.apiUrl = `AttendanceReport/GetDigitalGiftDisbursementReport?reportFormat=Pdf&userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&zoneCode=${this.zoneCode}&regionCode=${this.regionCode}&areaCode=${this.areaCode}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&empCode=${this.mioCode}`;
    debugger;
    this.commonService.GetCrystalReportData(this.apiUrl).subscribe((returns: any) => {
      let res = JSON.parse(returns);
      if (res.status) {
