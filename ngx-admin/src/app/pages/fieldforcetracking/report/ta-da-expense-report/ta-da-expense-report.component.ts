...
@Component({
  selector: 'ngx-ta-da-expense-report',
  templateUrl: './ta-da-expense-report.component.html',
  styleUrls: ['./ta-da-expense-report.component.scss']
})
export class TaDaExpenseReportComponent implements OnInit {
  ...
  pageNavigation = "TA DA & Expense Report";
  ...
  generateCrReport(reportFormat: any) {
    ...
    this.apiUrl = `AttendanceReport/GetTA_DA_Report?reportFormat=${reportFormat ?? 'Pdf'}&userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&zoneCode=${this.zoneCode}&regionCode=${this.regionCode}&areaCode=${this.areaCode}`;
