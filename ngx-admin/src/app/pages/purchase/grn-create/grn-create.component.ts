...
export class GrnCreateComponent implements OnInit {
  ...
  private save() {
    ...
    if (this.master.supplierChallanNo == null || this.master.supplierChallanNo == "" || this.master.supplierChallanNo == undefined) {
      this.toastrService.danger("Please enter Supplier Challan No.", "Message");
      this.commonService.valueSet("create");
      return false;
    }
    
    if (this.master.supplierChallanDate == null || this.master.supplierChallanDate == undefined) {
      this.toastrService.danger("Please enter Supplier Challan Date.", "Message");
      this.commonService.valueSet("create");
      return false;
    }
    ...
    for (let index = 0; index < this.master.lstDetailsViewModel.length; index++) {
      ...
      if (element.isSelect == 1) {  // 1 is YES
        ...
        this.master.lstDetailsViewModel[index].expiryDate = this.commonService.DateFormat(this.master.lstDetailsViewModel[index].expiryDate);
        this.master.lstDetailsViewModel[index].mfgDate = this.commonService.DateFormat(this.master.lstDetailsViewModel[index].mfgDate);
    ...
    this.master.factoryReceivedDate = this.commonService.DateFormat(this.master.factoryReceivedDate);
    this.master.supplierChallanDate = this.commonService.DateFormat(this.master.supplierChallanDate);
    console.log('GRN: ', this.master);

    let button = this.commonService.buttonClicked;
  ...
  public getAllPurchaseOrdersForGRN() {
    ...
    //debugger
    this.PurchaseorderService.getPurchaseOrdersForGRNN(0).subscribe((returns: any) => {
      if (returns.success) {
  ...
  getPurchaseOrderItems(event: any) {
    ...
    if (event) {
      this.master.purchaseOrderDate = event.purchaseOrderDate;
      this.master.partySelected = { id: event.supplierId, name: event.supplierName };
      this.master.purchaseOrderId = event.id;
      this.PurchaseorderService.getPODetailsByIdForGRN(0, this.master.purchaseOrderId).subscribe((returns: any) => {
        if (returns.success) {
  ...
  CalculateLineTotal(rowIndex: number) {
    ...
    lineTotal = Math.round(ttlQty * linePrice);
  ...
  generateCrReport(reportFormat: any, masterId: number) {
    ...
    const apiUrl = `PurchaseRequisition/GetGRNById?reportFormat=${reportFormat}&masterId=${masterId}`;
    console.log('Hit');
    debugger;
  ...
  initGrid(grnMasterId: any) {

    this.PurchaseorderService.GetGRN(grnMasterId, this.loadFromDateShow, this.loadToDateShow).subscribe((data: any) => {
      if (data.success) {
        this.rowData = data.data;
        //console.log(this.rowData);
      }
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.selectedRow = event.node.data;
      var grnMasterId = event.node.data.grnMasterId;
      //debugger;
      //this.getStore();
      this.PurchaseorderService.GetGRN(grnMasterId, this.loadFromDateShow, this.loadToDateShow).subscribe((data: any) => {
        if (data.success) {
          ...
          this.master.lstDetailsViewModel = [];
          this.PurchaseorderService.getPODetailsByIdForGRN(grnMasterId, this.master.purchaseOrderId).subscribe((returns: any) => {
            if (returns.success) {
              this.master.lstDetailsViewModel = returns.data;
            }
          });
  ...
  private agReport(event) {
    this.generateCrReport('Pdf', event.node.data.grnMasterId);
    //this.generateReport(event.data.grnMasterId);
  }

  private agDelete(event) {
    debugger
    this.master.grnMasterId = event.node.data.grnMasterId;
    if (event.node.data.grnStatus != 1) {
      if (confirm('Are you sure?')) {
        // var requsitionFinalMasterId = event.node.data.requisitionFinalizeMasterId;
        this.PurchaseorderService.DeleteGRNById(this.master.grnMasterId)
          .subscribe((returns: any) => {
            if (returns.success) {
      ...
      this.toastrService.danger("You can not delete.GRN is Approved!", "Message");
  ...
  public generateReport(grnMasterId) {
    ...
    this.PurchaseorderService.GetGRN(grnMasterId, this.loadFromDateShow, this.loadToDateShow).subscribe((data: any) => {
      if (data.success) {
        ...
        this.master.supplierChallanDate = data.data[0].supplierChallanDate;
        this.master.lstDetailsViewModel = [];
        this.PurchaseorderService.getPODetailsByIdForGRNforReport(grnMasterId, this.master.purchaseOrderId).subscribe((returns: any) => {
          if (returns.success) {
            this.master.lstDetailsViewModel = returns.data;
  ...
  gRowIndex: number = -1;
  ViewDetails(dialog: TemplateRef<any>, purchaseOrderDetailsId: number, rowIndex: number) {
    debugger;
