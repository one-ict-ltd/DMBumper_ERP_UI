...
@Component({
  selector: 'ngx-purchase-requisition-ho',
  templateUrl: './purchase-requisition-ho.component.html',
  styleUrls: ['./purchase-requisition-ho.component.scss']
})
export class PurchaseRequisitionHOComponent implements OnInit {
  /////////////////////////////
  master: {
  ...
  public pageNavigation = "Purchase Requisition For Head Office";
    ...
    this.purchaserequisitionService
      ...
      .subscribe((returns: any) => {
        if (returns.success) {
          ...
          //this.getProductReqNo();
          this.purchaserequisitionService
            .getPurchaseRequisitionById(null, 1)
            .subscribe((data: any) => {
              if (data.success) {
                //debugger;
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.getSBU(0);
      this.purchaserequisitionService
        .getPurchaseRequisitionById(purchaseReqId, 1)
        .subscribe((data: any) => {
          if (data.success) {
            //debugger;
    ...
    if (confirm('Are you sure to delete?')) {
      ...
      this.purchaserequisitionService.deletePurchaseReqById(this.master.purchaseReqId).subscribe((returns: any) => {
        if (returns.success) {
          ...
          //////////////Grid Refresh ///////////////////
          this.purchaserequisitionService.getPurchaseRequisitionById(null, 1).subscribe((data: any) => {
            if (data.success) {
              this.rowData = data.data;
            }
          });
  ...
  public getSBU(companyId) {
    ...
    this.comboService.getSBU(companyId).subscribe((returns: any) => {
      ...
      this.fromsbus = returns.data.filter(x => x.sbuId == 1).map((val) => ({
        id: val.sbuId,
        name: val.sbuName,
      }));

      if (this.fromsbus.length == 1) {
        this.master.fromsbusSelected = { id: this.fromsbus[0].id, name: this.fromsbus[0].name }
        this.master.fromWarehouseId = this.fromsbus[0].id
      }
    });
  }
  ...
  public getProductType() {
    this.productService.getProductType().subscribe((retuns: any) => {
      if (retuns.success) {
        //  debugger
        // Promotional Item =2 Office Equipment =5
        this.productTypeList = retuns.data.filter(x => x.productTypeId == 2 || x.productTypeId >= 5 || x.productTypeId == 3).map((val: any) => ({
  ...
  private agReport(event) {
    ...
    if (event.data.purchaseReqId != null && event.data.purchaseReqId > 0) {
      this.generateCrReport(event.data.purchaseReqId, 'pdf');
    }
  }

  generateCrReport(purchaseReqId: any, reportFormat: any) {
    let apiUrl = `PurchaseRequisition/GetPurchaseRequisitionRptData?purchaseRequisitionId=${purchaseReqId}&reportFormat=${reportFormat}&isHo=1`;
