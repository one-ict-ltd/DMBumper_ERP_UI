...
@Component({
  selector: 'ngx-grn-create-for-rejectedqty',
  templateUrl: './grn-create-for-rejectedqty.component.html',
  styleUrls: ['./grn-create-for-rejectedqty.component.scss']
})

export class GrnCreateForRejectedqtyComponent implements OnInit {
  ...
  public pageNavigation = "GRN Create From Return";
  ...
  private save() {
    ...
    if (this.master.rejectedGRN == "") {
      this.toastrService.danger("Please Select Rejected GRNNo!", "Message");
      this.commonService.valueSet("create");
      return false;
    }
    let flag = false;
  ...
  public getGRNNo() {
    ...
    //console.log("the finalrequsition date is:",this.master.grnDate)
    this.PurchaseorderService
      .getGRNNo(
  ...
  rejectedGRNList = [];
  public getAllRejectedGRNList(id: any) {
    this.rejectedGRNList = null;
    debugger
    this.PurchaseorderService.getRejectedGRN(id).subscribe((returns: any) => {
      if (returns.success) {

        this.rejectedGRNList = returns.data.map((val) => ({
          id: val.grnMasterId,
          name: val.grnNo
  ...
  public getAllPurchaseOrdersForGRN() {
    ...
    //debugger
    this.PurchaseorderService.getPurchaseOrdersForRejectedGRN(0).subscribe((returns: any) => {
      if (returns.success) {
  ...
  initGrid(grnMasterId: any) {
    ...
    this.PurchaseorderService.GetGRNForReturnOrder(grnMasterId).subscribe((data: any) => {
      if (data.success) {
        this.rowData = data.data;
        //console.log(this.rowData);
      }
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      //debugger;
      //this.getStore();
      this.PurchaseorderService.GetGRN(grnMasterId).subscribe((data: any) => {
        if (data.success) {
