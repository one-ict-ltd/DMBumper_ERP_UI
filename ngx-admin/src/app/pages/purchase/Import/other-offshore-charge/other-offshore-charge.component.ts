...
@Component({
  selector: 'ngx-other-offshore-charge',
  templateUrl: './other-offshore-charge.component.html',
  styleUrls: ['./other-offshore-charge.component.scss']
})
export class OtherOffshoreChargeComponent implements OnInit {
  ...
  master: {
    ...
    ImpOffshoreChargeId:number;
    OffshoreBankCharge:number;
    OffshoreBankChargeDate:Date;
    remarks:string
  };
  ...
  public pageNavigation = "Offshore Charge";
  ...
  public getMaster() {
    ...
    ImpOffshoreChargeId:0,
    OffshoreBankCharge:0,
    OffshoreBankChargeDate:new Date(),
    remarks:"",
    };
...
GetAreaByDepoCode(DepoCode);
  */
  ...
  private save() {
    ...
    debugger;
    this.master.OffshoreBankChargeDate=this.commonService.DateFormat(this.master.OffshoreBankChargeDate);
    this.purchaserequisitionService.SaveOffshoreChargeInfo(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
        ...
        this.show = true;
        //////////////Grid Refresh ///////////////////
        this.purchaserequisitionService.getOffshoreChargeInfo(0).subscribe((data: any) => {
          if (data.success) {
  ...
  constructor(
    ...
    this.columnDefs = [
      ...
      {
        headerName: "Offshore Bank Charge",
        field: "OffshoreBankCharge",
        filter: "agTextColumnFilter",
      },
      {
        headerName: "Offshore Bank Date",
        field: "OffshoreBankChargeDate",
        filter: "agTextColumnFilter",
      },
      {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.selectedRow = event.node.data;
      var chargeId = event.node.data.ImpOffshoreChargeId;

      this.purchaserequisitionService.getOffshoreChargeInfo(chargeId).subscribe((data: any) => {
        if (data.success) {
          debugger;
  ...
  private agDelete(event) {
    ...
    if (result) {
      //debugger;
      var chargeId = event.node.data.ImpOffshoreChargeId;
      this.purchaserequisitionService.deleteOffshoreChargeInfo(chargeId).subscribe((returns: any) => {
        if (returns.success) {
  ...
  public getPreLcAndLcData(id)
  {
    ...
    this.master.bankSelected=null,
     ...
     this.purchaserequisitionService.getPreLcandLcBankInsuranceOtherChargeInfoforOffshoreCharge(id).subscribe((returns:any)=>{
      if(returns.success)
      {
        ...
        this.master.CustomsDutyOthersCharge=returns.data[0].CustomsDutyOthersCharge;
        this.master.ClearingCNFCharge=returns.data[0].ClearingCNFCharge;
        this.master.LoadingUnloadingCharge=returns.data[0].LoadingUnloadingCharge;
        this.master.CarringCharge=returns.data[0].CarringCharge;
        this.master.OthersCharge=returns.data[0].OthersCharge;
        this.master.OthersCharge2=returns.data[0].OthersCharge2;

        this.master.CustomsDutyOthersChargeDate=returns.data[0].CustomsDutyOthersChargeDate;
        this.master.LoadingUnloadingChargeDate=returns.data[0].LoadingUnloadingChargeDate;
        this.master.ClearingCNFChargeDate=returns.data[0].ClearingCNFChargeDate;
        this.master.CarringChargeDate=returns.data[0].CarringChargeDate;
