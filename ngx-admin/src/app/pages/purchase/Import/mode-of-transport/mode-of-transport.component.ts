...
@Component({
  selector: 'ngx-mode-of-transport',
  templateUrl: './mode-of-transport.component.html',
  styleUrls: ['./mode-of-transport.component.scss']
})
export class ModeOfTransportComponent implements OnInit {
  ...
  master: {
    ...
    modeTransportName:string;
    modeTransportCode:string;
  };
  ...
  public pageNavigation = "Mode of Transport";
  ...
  private save() {
    if (this.master.modeTransportName == "") {
     ...
     this.purchaserequisitionService.saveModeOfTransport(this.master).subscribe((returns: any)=>{
      if(returns.success)
      {
        if(button=="update"){
        ...
        this.getMaster();
        this.purchaserequisitionService.getModeOfTransport(0).subscribe((data: any) => {
          if (data.success) {
  ...
  private agDelete(event) {
    debugger;
    this.master.modeTransportId = event.node.data.modeTransportId;
   

    this.purchaserequisitionService.deleteModeOfTransport(this.master).subscribe((returns:any)=>{
     if(returns.success){
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.purchaserequisitionService.getModeOfTransport(this.master.modeTransportId).subscribe((data:any)=>{
        if(data.success){
