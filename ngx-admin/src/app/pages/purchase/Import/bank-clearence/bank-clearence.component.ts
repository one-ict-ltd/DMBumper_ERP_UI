...
@Component({
  selector: 'ngx-bank-clearence',
  templateUrl: './bank-clearence.component.html',
  styleUrls: ['./bank-clearence.component.scss']
})
export class BankClearenceComponent implements OnInit {
  ...
  public pageNavigation = "Bank Clearence ";
  ...
  */
  ...
  private save() {
    ...
    this.purchaserequisitionService.saveClearanceInfo(this.master).subscribe((returns: any) => {
      if (returns.success) {
        ...
        this.show = true;
        //////////////Grid Refresh ///////////////////
        this.purchaserequisitionService.getClearenceInfo(0, 1).subscribe((data: any) => {
          if (data.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.purchaserequisitionService.getClearenceInfo(clearanceId, 1).subscribe((data: any) => {
        if (data.success) {
  ...
  public getReferenceList() {
    ...
    debugger;
    this.purchaserequisitionService.getReferceInfoforBankClearenceBasedOnShipmentInfo(0).subscribe((returns: any) => {
      if (returns.success) {
  ...
  public getShipmentLcPreLcData(id) {
    ...
    this.purchaserequisitionService.getPreLcLcShipmentInfoForBankClearence(id).subscribe((returns: any) => {
      if (returns.success) {
