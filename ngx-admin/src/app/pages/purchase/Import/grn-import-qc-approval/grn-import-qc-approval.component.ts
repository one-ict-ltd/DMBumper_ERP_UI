...
@Component({
  selector: 'ngx-grn-import-qc-approval',
  templateUrl: './grn-import-qc-approval.component.html',
  styleUrls: ['./grn-import-qc-approval.component.scss']
})

export class GrnImportQcApprovalComponent implements OnInit {
  ...
  public pageNavigation = "GRN Import QC Approval";
  ...
  SaveValidation(): boolean {
    debugger
    if (
      this.master.approvalStatus == null || this.master.approvalStatus == '0' || this.master.approvalStatus == ''
    ) {
    ...
    this.master.grnModel.forEach((e) => {
      if (e.isSelect == 1) count++; e.grnStatus = 1;
    });
  ...
  private save() {
    ...
    if (this.SaveValidation() == true) {
      this.PurchaseorderService
        .UpdateGRNImportQaForApproval(this.master)
  ...
  public GetGRNListForApproval() {
    debugger
    this.commonService.valueSet("create");
    this.PurchaseorderService.getGRNImportForQA().subscribe((returns: any) => {
      if (returns.success) {
        this.GRNList = returns.data.map((val: any) => ({
          ...
          ImpgrnMasterId: val.ImpgrnMasterId,
          InitialOrRetest: val.InitialOrRetest
        }));
      }
    });
  }
  public getGrnDetails(ImpgrnMasterId: any) {
    debugger
    this.GRNList.forEach((item: any) => {
      if (item.ImpgrnMasterId == ImpgrnMasterId) {
        ...
        this.master.MRRNo = item.MRRNo;
        this.master.RMRNo = item.MRRNo;
        this.master.ImpgrnMasterId = item.ImpgrnMasterId;
        this.master.InitialOrRetest = item.InitialOrRetest;
      }
    });
    this.PurchaseorderService.GetGrnImportDetailsForQA(ImpgrnMasterId, this.master.InitialOrRetest).subscribe((data1: any) => {
      debugger
      if (data1.success) {
