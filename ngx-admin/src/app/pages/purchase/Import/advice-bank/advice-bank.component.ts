...
@Component({
  selector: 'ngx-advice-bank',
  templateUrl: './advice-bank.component.html',
  styleUrls: ['./advice-bank.component.scss']
})
export class AdviceBankComponent implements OnInit {
  ...
  public pageNavigation = "Advice Bank";
  ...
  private save() {
    if (this.master.adviceBankName == "") {
     ...
     this.purchaserequisitionService.saveAdviceBank(this.master).subscribe((returns: any)=>{
      if(returns.success)
      {
        if(button=="update"){
        ...
        this.getMaster();
        this.purchaserequisitionService.getAdviceBank(0).subscribe((data: any) => {
          if (data.success) {
  ...
  private agDelete(event) {
    debugger;
    this.master.adviceBankId = event.node.data.adviceBankId;
   

    this.purchaserequisitionService.deleteAdviceBank(this.master).subscribe((returns:any)=>{
     if(returns.success){
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.purchaserequisitionService.getAdviceBank(this.master.adviceBankId).subscribe((data:any)=>{
        if(data.success){
