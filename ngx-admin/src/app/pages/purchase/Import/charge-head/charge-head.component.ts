...
@Component({
  selector: 'ngx-charge-head',
  templateUrl: './charge-head.component.html',
  styleUrls: ['./charge-head.component.scss']
})
export class ChargeHeadComponent implements OnInit {
  ...
  public pageNavigation = "Charge Head";
  ...
  private save() {
    if (this.master.chargeHeadName == "") {
     ...
     this.purchaserequisitionService.saveChargeHead(this.master).subscribe((returns: any)=>{
      if(returns.success)
      {
        if(button=="update"){
        ...
        this.getMaster();
        this.purchaserequisitionService.getChargeHead(0).subscribe((data: any) => {
          if (data.success) {
  ...
  private agDelete(event) {
    this.master.chargeHeadId = event.node.data.chargeHeadId;
    this.hrmmasterService.deleteGender(this.master).subscribe((returns: any) => {
      if (returns.success) {
        this.toastrService.success(this.commonService.deletedmsg, "Message");

        //////////////Grid Refresh ///////////////////
        this.hrmmasterService.getGender(0).subscribe((data: any) => {
          if (data.success) {
            this.rowData = data.data;
          }
        });
    ...
    this.purchaserequisitionService.deleteChargeHead(this.master).subscribe((returns:any)=>{
     if(returns.success){
      this.toastrService.success(this.commonService.deletedmsg, "Message");
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.purchaserequisitionService.getChargeHead(this.master.chargeHeadId).subscribe((data:any)=>{
        if(data.success){
          this.master=data.data[0];
        }
      })
