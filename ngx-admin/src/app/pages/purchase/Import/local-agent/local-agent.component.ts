...
@Component({
  selector: 'ngx-local-agent',
  templateUrl: './local-agent.component.html',
  styleUrls: ['./local-agent.component.scss']
})
export class LocalAgentComponent implements OnInit {
  ...
  public pageNavigation = "Lcoal Agent";
  ...
  private save() {
    if (this.master.localAgentName == "") {
     ...
     this.purchaserequisitionService.saveLocalAgent(this.master).subscribe((returns: any)=>{
      if(returns.success)
      {
        if(button=="update"){
        ...
        this.getMaster();
        this.purchaserequisitionService.getLocalAgent(0).subscribe((data: any) => {
          if (data.success) {
  ...
  private agDelete(event) {
    debugger;
    this.master.localAgentId = event.node.data.localAgentId;
   

    this.purchaserequisitionService.deleteLocalAgent(this.master).subscribe((returns:any)=>{
     if(returns.success){
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.purchaserequisitionService.getLocalAgent(this.master.localAgentId).subscribe((data:any)=>{
        if(data.success){
