...
@Component({
  selector: 'ngx-purchase-requ-file-upload',
  templateUrl: './purchase-requ-file-upload.component.html',
  styleUrls: ['./purchase-requ-file-upload.component.scss']
})

export class PurchaseRequFileUploadComponent implements OnInit {
  ...
  public pageNavigation = "Purchase Requisition File Upload";
  ...
  public ButtonAction() {
    if (this.commonService.buttonClicked == "create") {
      ...
      this.toastrService.info('No view available!', 'Message');
      return;
    } else if (this.commonService.buttonClicked == "reset") {
      this.reset();
    } else if (this.commonService.buttonClicked == "edit") {
      //this.edit();
      this.toastrService.info('Edit can not be performed!', 'Message');
      return;
    }
  }
  public getMaster() {
  ...
  private save() {
    ...
    if (this.allStatusOk == false) {
      this.toastrService.danger("Please verify data first!", "Message");
  ...
  public async onRowClicked(event) {
    ...
    if (data == "edit") {
      this.toastrService.info("Edit not available!", "Message");
      return false;
      ...
      this.toastrService.info("View not available!", "Message");
      return false;
      // this.agEdit(event);
      // this.show = false;
      // this.disabled = true;
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = async (e: any) => {
      ...
      this.totalRowsOfFile = this.data.length;
      ...
      for (let start = 0; start < this.data.length; start += chunkSize) {
        ...
        chunk.forEach(e => {
          ...
          for (let k = 0; k < header.length; k++) {
            const requQty = e[k + 3];
            if (!isNaN(requQty) && parseFloat(requQty) !== 0 && requQty !== '') {
              debugger
              this.lstdetailmodelT = [];
              let detail = {
                purchaseReqDetailsId: 0,
  ...
  public VerifyData() {
    ...
    if (this.master.productTypeId == null || this.master.productTypeId == undefined || this.master.productTypeId == 0) {
    ...
    setTimeout(() => {
      ...
      let productTypeId = 0, verificatioCounter = 0;
      this.master.lstReqDetailsViewModel.forEach((el, index) => {

        this.salesinvoiceService.getProductWiseSpecificationIdByName(el.productCode).subscribe((returns: any) => {
          if (returns.success) {
            debugger
         //   console.log(returns.data);
            if (returns.data[0].errorMessage == null) {
              verificatioCounter++;
              el.productName = returns.data[0].productName;
              el.uomName = returns.data[0].uomName;
              el.currentStockQty = 0;//returns.data[0].currentStock;
              el.productWiseSpecificationId = returns.data[0].productWiseSpecificationId;
              el.uomId = returns.data[0].uomId;
              el.productId = returns.data[0].productId;
              productTypeId = returns.data[0].productTypeId;

              if (productTypeId != this.master.productTypeId) {
                this.toastrService.warning(`Selected product type and upload product type is not matching at row number ${index + 1}.`, 'Message');
                this.allStatusOk = false;
                return;
              }
              if (verificatioCounter == this.totalRowsOfFile) {
            ...
            if (returns.data[0].errorMessage != null) {
              this.toastrService.danger(`Invalid Product Code at row number ${index + 1}`);
              this.allStatusOk = false;
              return;
            }
      ...
      if (this.validRowCounter == this.master.lstReqDetailsViewModel.length) {
        this.verifyStatus = "Verified";
        this.allStatusOk = true;
