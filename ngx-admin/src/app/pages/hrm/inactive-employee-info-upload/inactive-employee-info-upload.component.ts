...
@Component({
  selector: 'ngx-inactive-employee-info-upload',
  templateUrl: './inactive-employee-info-upload.component.html',
  styleUrls: ['./inactive-employee-info-upload.component.scss']
})
export class InactiveEmployeeInfoUploadComponent implements OnInit {
  ...
  public pageNavigation = " Inactive Employee's Information  Upload";//"Employee's Salary Fixed Head Structure";
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      this.data.forEach(e => {
        let inActiveDate = new Date(e[1]);
        console.log('inactive Date:', e[1])
        debugger;
        let obj = {
          //EmpFixedHeadStructureId: 0,
          employeeId: 0,
        ...
        console.log(this.master.lstMaster);
      });
  ...
  public UploadData() {
    ...
    this.employeeinformationService.SaveInactiveEmployeeInfofromExcel(this.master.lstMaster).subscribe((retuns: any) => {
      if (retuns.status) {
  ...
  public VerifyData() {
    ...
    this.employeeinformationService.GetInactiveEmployeeInfoUploadDataVerify(this.master.lstMaster).subscribe((retuns: any) => {
      if (retuns.status) {
        this.master.lstMaster = retuns.data
        //this.master.lstMaster=retuns.data.
        // console.log("verify data list is =====================",this.master.lstMaster);
        this.totalData = this.master.lstMaster.length;
        let vData = this.master.lstMaster.filter((i: any) =>i.status == "OK").length;
