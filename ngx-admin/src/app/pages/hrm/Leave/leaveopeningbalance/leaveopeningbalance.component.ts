...
@Component({
  selector: 'ngx-leaveopeningbalance',
...
export class LeaveopeningbalanceComponent implements OnInit {
  ...
  public pageNavigation = "Leave Opening Balance";
  ...
  public getDuplicate() {
    //debugger;
    this.leaveService.getDuplicateLeaveOpeningBalance(this.master.leaveOpeningBalanceId, this.master.yearId, this.master.leaveTypeId, this.master.employeeId).subscribe((returns: any) => {
      //debugger;
  ...
  private save() {
    ...
    else if (this.master.leaveDays == 0 || this.master.leaveDays == null) {
      this.toastrService.danger("Please type yearly maximum leave ", "Message");
      this.commonService.valueSet("create");
      return false;
    }
    else if (this.master.countData != 0) {
      this.toastrService.danger("Duplicate leave Opening Balance", "Message");
      this.commonService.valueSet("create");
      return false;
    }
    //alert(this.master.yearlyMaxLeave);
    this.leaveService.saveLeaveOpeningBalance(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
        ...
        this.show = true;
        //////////////Grid Refresh ///////////////////
        this.leaveService.getLeaveOpeningBalance().subscribe((data: any) => {
          if (data.success) {
  ...
  constructor(
    ...
    this.columnDefs = [
      ...
      {
        headerName: "Leave Carry Day",
        field: "leaveCarryDays",
        filter: "agTextColumnFilter",
      },
      {
  ...
  private agDelete(event) {
    ...
    if (result) {
      this.master.leaveOpeningBalanceId = event.node.data.leaveOpeningBalanceId;
