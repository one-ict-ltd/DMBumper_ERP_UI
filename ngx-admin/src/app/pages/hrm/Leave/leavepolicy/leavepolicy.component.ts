...
export class LeavepolicyComponent implements OnInit {
  ...
  master: {
    ...
    yearlyMaxCarry: number;
    remarks: string;
    weeklyOffBridge: number;
    govtHolidayBridge: number;
    paymentType: string;
    highestCarryForward: number;
  ...
  public pageNavigation = "Leave Policy";
  ...
  public getDuplicate() {
    //debugger;
    this.leaveService.getDuplicateLeavePolicy(this.master.leavePolicyId, this.master.yearId, this.master.leaveTypeId).subscribe((returns: any) => {
      //debugger;
  ...
  private save() {
    ...
    else if (this.master.yearlyMaxLeave == 0 || this.master.yearlyMaxLeave == null) {
      this.toastrService.danger("Please type yearly maximum leave ", "Message");
      this.commonService.valueSet("create");
      return false;
    }
    else if (this.master.countData != 0) {
      this.toastrService.danger("Duplicate leave policy", "Message");
      this.commonService.valueSet("create");
      return false;
    }
    //alert(this.master.yearlyMaxLeave);
    this.leaveService.saveLeavePolicy(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
        ...
        this.show = true;
        //////////////Grid Refresh ///////////////////
        this.leaveService.getLeavePolicy().subscribe((data: any) => {
          if (data.success) {
  ...
  constructor(
    ...
    this.columnDefs = [
      ...
      {
        headerName: "Max leave",
        field: "yearlyMaxLeave",
        filter: "agTextColumnFilter",
      },
      {
        headerName: "Max Carry",
        field: "yearlyMaxCarry",
        filter: "agTextColumnFilter",
      },
      {
        headerName: "weekly Bridge",
        field: "weeklyOffBridge",
      },
      {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.selectedRow = event.node.data;
      var leavePolicyId = event.node.data.leavePolicyId;

      this.leaveService.getLeavePolicyById(leavePolicyId).subscribe((data: any) => {
        if (data.success) {
          //debugger;
  ...
  private agDelete(event) {
    ...
    if (result) {
      this.master.leavePolicyId = event.node.data.leavePolicyId;
      this.leaveService.deleteLeavePolicy(this.master).subscribe((returns: any) => {
        if (returns.success) {
