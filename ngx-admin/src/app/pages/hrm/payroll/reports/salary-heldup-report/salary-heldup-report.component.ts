...
@Component({
  selector: 'ngx-salary-heldup-report',
  templateUrl: './salary-heldup-report.component.html',
  styleUrls: ['./salary-heldup-report.component.scss']
})

export class SalaryHeldupReportComponent implements OnInit {

  pageNavigation = "Salary Heldup Report";
  rReportHeader = "Salary Heldup Report";
  fiscalYearId: number;
  monthName: string;
  ...
  generateCrReport(reportFormat: any) {
    ...
    this.apiUrl = `SalaryReport/RptSalaryHeldupReport?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&heldupMonth=${this.commonService.DateFormat(heldupMonth)}&salaryPeriodId=${null}&departmentId=${this.departmentId}&salaryLocationId=${this.salaryLocation}`;
  ...
  private getReportData() {
    ...
    if (this.DepartmentSelected != null) {
      this.departmentId = this.DepartmentSelected[0].id;
    }

    if (this.salaryLocationSelected != null) {
      this.salaryLocation = this.salaryLocationSelected[0].id;
    }

    this.apiUrl = "";
    let userInfo = this.commonService.GetUserProfileJson();
    this.apiUrl = `SalaryReport/RptSalaryHeldupReport?reportFormat=Pdf&userId=${userInfo[0].employeeid}&heldupMonth=${this.commonService.DateFormat(heldupMonth)}&salaryPeriodId=${null}departmentId=${this.departmentId}&salaryLocationId=${this.salaryLocation}`;
  ...
  private onExportCSV() {
    ...
    this.commonService.GenerateCSVWithoutPhoneWebsite(cellDataArray, this.tableHeader, this.rReportHeader, headerParams, "");
  }
