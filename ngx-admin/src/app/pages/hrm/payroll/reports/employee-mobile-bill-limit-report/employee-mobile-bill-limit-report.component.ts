...
@Component({
  selector: 'ngx-employee-mobile-bill-limit-report',
  templateUrl: './employee-mobile-bill-limit-report.component.html',
  styleUrls: ['./employee-mobile-bill-limit-report.component.scss']
})
export class EmployeeMobileBillLimitReportComponent implements OnInit {
  ...
  public RptButtonAction() {
    ...
    if (clicked == "preview") {
      this.toastrService.info('Preview Not available, please click print or Excel', 'Message');
      return;
      this.onPreview();
    } else if (clicked == "pdf") {
  ...
  public generateReport(buttonAction: any) {
    ...
    this.generateCrReport(0, 0, 'pdf');
  }
  generateCrReport(userId: number, employeeId: number, reportFormat: any) {
    debugger
    let apiUrl = `Pims/GetEmployeeMobileBillInfo?userId=${0}&employeeId=${0}&reportFormat=${reportFormat}`;
  ...
  private onExportCSV() {
    ...
    this.generateCrReport(0, 0, 'Excel')
  }

  public GetRptSalarySheetPdf() {

    this.salaryreportService.GetEmployeeMobileBillInfo(0, 0, 'Pdf').subscribe((returns: any) => {
      this.commonService.GenerateBase64ToReport(returns);
    });
  }
