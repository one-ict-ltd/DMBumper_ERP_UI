...
export class EmployeetypeComponent implements OnInit {
  ...
  public pageNavigation = "Employee Type";
  ...
  private save() {
    if (this.master.empType == "") {
      ...
      this.hrmmasterService.saveEmployeeType(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.hrmmasterService.getEmployeeType(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    this.master.employeeTypeId = event.node.data.employeeTypeId;
    this.hrmmasterService.deleteEmployeeType(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.hrmmasterService.getEmployeeType(this.master.employeeTypeId).subscribe((data: any) => {
        if (data.success) {
