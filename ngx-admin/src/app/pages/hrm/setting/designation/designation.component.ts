...
export class DesignationComponent implements OnInit {
  ...
  public pageNavigation = "Designation";
  ...
  private save() {
    if (this.master.designationName == "") {
      ...
      this.hrmmasterService.saveDesignation(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.hrmmasterService.getDesignation(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    this.master.designationId = event.node.data.designationId;
    this.hrmmasterService.deleteDesignation(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.hrmmasterService.getDesignation(this.master.designationId).subscribe((data: any) => {
        if (data.success) {
