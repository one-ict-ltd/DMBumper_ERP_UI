...
export class ActivitytypeComponent implements OnInit {
  ...
  public pageNavigation = "Activity Type";
  ...
  private save() {
    if (this.master.activityTypeName == "") {
      ...
      this.hrmmasterService.saveActivityType(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.hrmmasterService.getActivityType(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    this.master.activityTypeId = event.node.data.activityTypeId;
    this.hrmmasterService.deleteActivityType(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.hrmmasterService.getActivityType(this.master.activityTypeId).subscribe((data: any) => {
        if (data.success) {
