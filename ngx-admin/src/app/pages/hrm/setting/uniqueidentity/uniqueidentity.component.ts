...
@Component({
   selector: 'ngx-uniqueidentity',
   templateUrl: './uniqueidentity.component.html',
   styleUrls: ['./uniqueidentity.component.scss']
})
export class UniqueidentityComponent implements OnInit {
  ...
  public pageNavigation = "Unique Identity";
  ...
  private save() {
    if (this.master.Name == "") {
      ...
      this.hrmmasterService.saveUniqueIdentity(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.hrmmasterService.getUniqueIdentity(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    this.master.uniqueIdentityId = event.node.data.uniqueIdentityId;
    this.hrmmasterService.deleteUniqueIdentity(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.hrmmasterService.getUniqueIdentity(this.master.uniqueIdentityId).subscribe((data: any) => {
        if (data.success) {
