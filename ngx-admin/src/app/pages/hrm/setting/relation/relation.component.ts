...
export class RelationComponent implements OnInit {
  ...
  public pageNavigation = "Relation";
  ...
  private save() {
    if (this.master.relationName == "") {
      ...
      this.hrmmasterService.saveEmployeeRelation(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.hrmmasterService.getEmployeeRelation(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    this.master.relationId = event.node.data.relationId;
    this.hrmmasterService.deleteEmployeeRelation(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.hrmmasterService.getEmployeeRelation(this.master.relationId).subscribe((data: any) => {
        if (data.success) {
