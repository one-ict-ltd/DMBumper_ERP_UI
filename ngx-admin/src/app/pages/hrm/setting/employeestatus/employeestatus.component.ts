...
export class EmployeestatusComponent implements OnInit {
  ...
  public pageNavigation = "Employee Status";
  ...
  private save() {
    if (this.master.statusName == "") {
      ...
      this.hrmmasterService.saveEmployeeStatus(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.hrmmasterService.getEmployeeStatus(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    this.master.employeeStatusId = event.node.data.employeeStatusId;
    this.hrmmasterService.deleteEmployeeStatus(this.master.employeeStatusId).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.hrmmasterService.getEmployeeStatus(this.master.employeeStatusId).subscribe((data: any) => {
        if (data.success) {
