...
export class GenderComponent implements OnInit {
  ...
  public pageNavigation = "Employee Gender";
  ...
  private save() {
    if (this.master.Name == "") {
      ...
      this.hrmmasterService.saveGender(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
  ...
  private agDelete(event) {
    this.master.genderId = event.node.data.genderId;
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.hrmmasterService.getGender(this.master.genderId).subscribe((data: any) => {
        if (data.success) {
