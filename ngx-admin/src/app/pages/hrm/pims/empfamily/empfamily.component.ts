...
@Component({
  selector: 'ngx-empfamily',
  templateUrl: './empfamily.component.html',
  styleUrls: ['./empfamily.component.scss']
})
export class EmpfamilyComponent implements OnInit {
    ...
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      this.employeeinformationService.GetEmployeeBasicInfoById(this.employeeId).subscribe((data: any) => {
        if (data.success) {
          ...
          this.pageNavigation = this.employeeName + "'s Family Information";
        }
      });
    });
  ...
  public relationItems=[];
  public getDropdownData() {
    this.employeeotherinfoService.GetEmployeeAllRelation().subscribe((returns: any) => {
      this.relationItems = returns.data.map((val) => ({
        id: val.relationId,
...
//  public GetEmployeeAllRelations(){
//   this.employeeotherinfoService.GetEmployeeAllRelation().subscribe((returns:any)=>{
//     if(returns.success){
//       this.relationItems = returns.data.map((val: any) => ({
//         id: val.relationId,
//         name: val.name,
//       }))
//     }
...
//  }
  ...
  private save() {
    ...
    if (this.master.email && this.master.email !== "" && !emailRegex.test(this.master.email)) {
  ...
  onGridReady(params) {
    ...
    this.employeeotherinfoService.GetEmployeeFamilyInfoById(0, this.master.employeeId).subscribe((data: any) => {
      if (data.status) {
        this.rowData = data.data;
      }
    });
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.employeeotherinfoService.GetEmployeeFamilyInfoById(familyInfoId, employeeId).subscribe((data: any) => {
        if (data.status) {
