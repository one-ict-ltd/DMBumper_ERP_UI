...
@Component({
  selector: 'ngx-weekly-task-assign',
  templateUrl: './weekly-task-assign.component.html',
  styleUrls: ['./weekly-task-assign.component.scss']
})
export class WeeklyTaskAssignComponent implements OnInit {
...
monthList = [
  ...
  public pageNavigation = "Weekly Task Assign";
...
public addDetails(dialog: TemplateRef<any>) {
  ...
  private save() {
    ...
    this.taskManagementService.SaveEmployeeWeeklyTaskAssign(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
  ...
  public GetEmployeeMonthlyTaskAssignByYearMonthTeamMemberEmployeeId() {
    this.taskManagementService.GetEmployeeMonthlyTaskAssignByYearMonthTeamMemberEmployeeId(0, this.employeeId,this.master.teamMemberEmployeeId,this.master.year,this.master.month).subscribe((data: any) => {
      if (data.status) {
