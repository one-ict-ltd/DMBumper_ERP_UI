...
@Component({
  selector: 'ngx-attendance-summary-report',
...
export class AttendanceSummaryReportComponent implements OnInit {
  ...
  public getMaster() {
    this.master = {
      companyId: 0,
      startDate: this.commonService.GetFirstDateOfMonth(new Date()),
      endDate: new Date(),
      departmentId: 0,
      sbuId: 0,
    };
  ...
  public pageNavigation = "Attendance Summary Report";
  rReportHeader = "Attendance Summary Report";
  public tableHeader = [
    "#",
    "Emp. ID",
    "Name",
  ...
  private getGridData() {
    ...
    if (this.master.startDate == null) {
      this.toastrService.danger("Please select a from date", "Message");
      return false;
    }
    if (this.master.endDate == null) {
      this.toastrService.danger("Please select a end date", "Message");
      return false;
    }
    ...
    this.processattendanceService
      .GetAttendanceSummaryByDateRange(
        this.master.companyId,
        this.master.sbuId,
        this.master.departmentId,
        0,
        this.commonService.DateFormat(this.master.startDate),
        this.commonService.DateFormat(this.master.endDate),
      )
      .subscribe((data: any) => {
        debugger;
        if (data.status) {
