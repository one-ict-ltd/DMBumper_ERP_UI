...
@Component({
  selector: 'ngx-employee-cash-salary-setup',
  templateUrl: './employee-cash-salary-setup.component.html',
  styleUrls: ['./employee-cash-salary-setup.component.scss']
})
export class EmployeeCashSalarySetupComponent implements OnInit {
  ...
  public pageNavigation = "Employee Cash Salary Set-Up";
  ...
  private save() {
    ...
    this.poductPricingService
      .SaveCashSetUp(this.ProductPricingViewModel)
  ...
  validatePrice(index) {
    ...
    if (this.master.lstDetailsViewModel[index].cashAmount > 0) {
      this.ProductPricingViewModel = this.master.lstDetailsViewModel[index];
      this.save();
    } else {
      this.toastrService.warning(
        "Cash should be larger than 0 (ZERO)!",
        "Warning !"
      );
    }
  }
