...
@Component({
  selector: 'ngx-upload-salary-structure',
  templateUrl: './upload-salary-structure.component.html',
  styleUrls: ['./upload-salary-structure.component.scss']
})
export class UploadSalaryStructureComponent implements OnInit {
  ...
  public pageNavigation = "Employee's Salary Structure Upload";//"Employee's Salary Fixed Head Structure";
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      this.data.forEach(e => {
        let joiningdate = new Date(e[2]);
        console.log('Joining Date:', e[2])
        debugger;
        let obj = {
          EmpSalaryStructureId: 0,
          employeeId: 0,
  ...
  public UploadData() {
    ...
    this.SalaryFixedHeadStructureService.SaveEmployeeSalaryStructureUpload(this.master.lstMaster).subscribe((retuns: any) => {
      if (retuns.status) {
  ...
  public VerifyData() {
    ...
    this.SalaryFixedHeadStructureService.GetEmployeeSalaryStructureUploadDataVerify(this.master.lstMaster).subscribe((retuns: any) => {
      if (retuns.status) {
        this.master.lstMaster = retuns.data

        this.totalData = this.master.lstMaster.length;
        let vData = this.master.lstMaster.filter((i: any) =>
          i.status == "Emp-OKLocation-OKGrade-OK").length;
