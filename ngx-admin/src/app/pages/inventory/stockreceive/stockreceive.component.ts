...
@Component({
  selector: "ngx-stockreceive",
  templateUrl: "./stockreceive.component.html",
  styleUrls: ["./stockreceive.component.scss"],
})
export class StockreceiveComponent implements OnInit {
  ...
  public pageNavigation = "CSD Received [Stock Receive From Factory]";
  public rptHeader = "CSD Received";
  ...
  public pageNavigationreport = "Stock Received";
  public tableHeader = [
    "#",
    "Product Name",
    "Pack Size",
  ...
  generateCrReport(reportFormat: any, stockReceiveId: any) {
    ...
    let reportHeaderName = "Stock Received From CWH"; //"CSD Received"
    ...
    this.stockreceive
      ...
      .subscribe((returns: any) => {
        if (returns.success) {
          ...
          this.transferFrom = this.bodyData[0].transferFrom;
          this.setParam();
  ...
  public getSBU(companyId) {
    ...
    this.comboService.getSBU(companyId).subscribe((returns: any) => {
      if (returns.status) {
        ...
        //console.log(this.ReceiveSBUList)
        this.ReceiveSBUList = this.ReceiveSBUList.filter((element) => {
          //return element.id != 19 && element.id != 32;
  ...
  public getCurrentStock(index: number) {
    ...
    if (this.master.lstDetailsViewModel.length > 0) {
      ...
      productWiseSpecificationId = this.master.lstDetailsViewModel[index].productWiseSpecificationId;
      ...
      this.salesinvoiceService.GetCurrentStock(storeId, productWiseSpecificationId, batchNo).subscribe((returns: any) => {
        if (returns.success) {
          currentStock = returns.data[0].length == 0 ? 0 : returns.data[0].currentStock;
          this.master.lstDetailsViewModel[index].CurrentStock = currentStock;
        }
      });
    }
  ...
  public GetAllProductReceiveNumber(sbuId) {
    this.master.productTransferNoSelected = {};
    this.stockreceive
      .GetAllProductReceiveNumber(sbuId, this.master.receiveType)//'F2D')
      .subscribe((returns: any) => {
  ...
  public getProductTransferStockDetails(productReqId) {
