...
export class StockinreportComponent implements OnInit {
  public pageNavigation = "Stock In Stock";
  ...
  private getReportData() {
    ...
    // var storeNamehiden = "";
    this.bodyData = [];
    this.apiUrl = `stock/getStockInReport?productId=${this.master.productId}&productWiseSpecificationId=${this.master.productWiseSpecificationId}&companyId=${this.master.companyId}&sbuId=${this.master.sbuId}&storeId=${this.master.storeId}&isStoreWiseGroup=${this.master.isStoreWiseGroup}`;
    this.commonService.getReportData(this.apiUrl).subscribe((returns: any) => {
      if (returns.success) {
        this.bodyData = returns.data;
        // this.index=0;
        // specdata.map(item => {
        //   if (item.storeName != storeNamehiden) {
        //     hide = false;
        //   }
        //   else {
        //     hide = true;
        //   }
        //   this.bodyData.push({
        //     productName: item.productName, storeName: item.storeName, CurrentStock: item.CurrentStock, Name: item.Name, hide: hide
        //   });
        //   this.index++;
        //   storeNamehiden = item.storeName;
        // });
      } else {
