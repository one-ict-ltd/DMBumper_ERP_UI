...
@Component({
  selector: 'ngx-promo-disburse-details-report',
  templateUrl: './promo-disburse-details-report.component.html',
  styleUrls: ['./promo-disburse-details-report.component.scss']
})
export class PromoDisburseDetailsReportComponent implements OnInit {
  ...
  pageNavigation = "Promo Disburse Details Report";
  apiUrl = "";
  bodyData: any = [];
  ...
  generateCrReport(reportFormat: any) {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      ...
      this.apiUrl = `PromoReport/PromoDisburseDetailsReport?userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&depotCode=${depotCode}&territoryCode=${territoryCode}&reportFormat=${reportFormat}`;
