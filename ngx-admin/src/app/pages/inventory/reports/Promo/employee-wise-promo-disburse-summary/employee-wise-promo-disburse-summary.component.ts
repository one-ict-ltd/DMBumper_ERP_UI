...
@Component({
  selector: 'ngx-employee-wise-promo-disburse-summary',
  templateUrl: './employee-wise-promo-disburse-summary.component.html',
  styleUrls: ['./employee-wise-promo-disburse-summary.component.scss']
})
export class EmployeeWisePromoDisburseSummaryComponent implements OnInit {
  ...
  generateCrReport(reportFormat: any) {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      ...
      this.apiUrl = `PromoReport/PromoDisburseSummaryReportEmployeeWise?userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&reportFormat=${reportFormat}`;
