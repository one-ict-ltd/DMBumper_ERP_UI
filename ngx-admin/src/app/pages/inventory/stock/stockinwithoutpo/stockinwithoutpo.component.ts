...
@Component({
  selector: "ngx-stockinwithoutpo",
  templateUrl: "./stockinwithoutpo.component.html",
  styleUrls: ["./stockinwithoutpo.component.scss"],
})
export class StockinwithoutpoComponent implements OnInit {
  public company: {
    name: string;
    address: string;
  ...
  public pageNavigation = "Receiving Finish Goods From Purchase"; //"Stock In By OB Or Without PO";
  ...
  private save() {
    ...
    this.StockinwithoutpoService.saveStockIn(this.master).subscribe(
      (returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          this.StockinwithoutpoService.getStockInWithOtPO(0).subscribe(
            (data: any) => {
              if (data.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.StockinwithoutpoService.getStockInWithOtPO(stockMasterId).subscribe(
        (data: any) => {
          if (data.success) {
      ...
      this.StockinService.deleteStockInById(this.master.stockMasterId).subscribe(
        (returns: any) => {
          if (returns.success) {
            ...
            //////////////Grid Refresh ///////////////////
            this.StockinwithoutpoService.getStockInWithOtPO(
  ...
  public addStockdetails() {
    ...
    let detail = {
      ...
      purchaseRate: this.master.purchaseRate,
  ...
  public tableHeader = ["#", "Product Name", "Batch No.", "Stock Qty.", "Unit Price", "Current Stock"];
  ...
  public generateStockInReport(stockMasterId) {
    this.StockinwithoutpoService.getStockInWithOutPoReportById(
      ...
      if (returns.success) {
        ...
        this.purchaseOrderNo = this.bodyData[0].purchaseOrderNo;
        this.challanNo = this.bodyData[0].challanNo;
        this.lcNo = this.bodyData[0].lcNo;
        this.supplierName = this.bodyData[0].supplierName;
        this.setParam();
