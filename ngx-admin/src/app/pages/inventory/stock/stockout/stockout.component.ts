...
export class StockoutComponent implements OnInit {
  ...
  public pageNavigation = "Stock Out";
  ...
  public getMaster() {
    this.master = {
      ...
      productWiseSpecificationId: 0,
      stockQty: 0,
      stockCategory: "With Out Purchase",
      stockNo: "",
      stockDate: new Date(),
      stockTypeId: "",
      remarks: "",
  ...
  private save() {
    ...
    if (this.master.stockDate == null) {
      this.toastrService.danger("Please select SR date.", "Message");
      // this.show = false;
      this.commonService.valueSet("create");
      return false;
    }
    ...
    this.StockoutService.saveStockOut(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
        ...
        this.StockoutService.getStockOut(0).subscribe((data: any) => {
          if (data.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.StockoutService.getStockOut(stockMasterId).subscribe((data: any) => {
        if (data.success) {
  ...
  public getStockDetailsData(stockMasterId) {
    this.StockoutService.getStockDetailsOut(stockMasterId).subscribe(
      (data: any) => {
        if (data.success) {
  ...
  private agDelete(event) {
    ...
    this.StockoutService.deleteStockOutById(
      ...
      if (returns.success) {
        ...
        //////////////Grid Refresh ///////////////////
        this.StockoutService.getStockOut(this.master.stockMasterId).subscribe(
          (data: any) => {
            if (data.success) {
              this.rowData = data.data;
            }
  ...
  public getMaxPurchaseorderno() {
    this.StockoutService.getmaxSRNo("").subscribe((returns: any) => {
      if (returns.success) {
  ...
  public addStockOutDetails() {
    ...
    var indexu = this.master.stockDetailsList.findIndex(
      (x) =>
        x.productWiseSpecificationId == this.master.productWiseSpecificationId
    );
    if (indexu > -1) {
      this.master.stockDetailsList[indexu] = detail;
    } else {
  ...
  public getAllProductSpecification(productId) {
    ...
    this.StockinwithoutpoService.getAllProductSpecificationForStockInStockOut(
      productId
    ).subscribe((returns: any) => {
  ...
  public generateStockInReport(stockMasterId) {
    this.StockoutService.getStockOutReportById(stockMasterId).subscribe(
      (returns: any) => {
        if (returns.success) {
