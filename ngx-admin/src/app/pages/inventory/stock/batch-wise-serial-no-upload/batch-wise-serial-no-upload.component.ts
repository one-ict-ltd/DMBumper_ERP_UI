...
@Component({
  selector: 'ngx-batch-wise-serial-no-upload',
  templateUrl: './batch-wise-serial-no-upload.component.html',
  styleUrls: ['./batch-wise-serial-no-upload.component.scss']
})
export class BatchWiseSerialNoUploadComponent implements OnInit {
  ...
  public pageNavigation = "Batch Wise Serial No Upload";//"Employee's Salary Fixed Head Structure";
  ...
  public UploadData() {
    ...
    this.SalaryFixedHeadStructureService.SaveBatchWiseSerialNoUpload(this.master.lstMaster).subscribe((retuns: any) => {
      if (retuns.status) {
  ...
  public VerifyData() {
    ...
    this.SalaryFixedHeadStructureService.GetBatchWiseSerialNoUploadDataVerify(this.master.lstMaster).subscribe((retuns: any) => {
      if (retuns.status) {
        this.master.lstMaster = retuns.data

        this.totalData = this.master.lstMaster.length;
        let vData = this.master.lstMaster.filter((i: any) =>
          i.status == "SKUNumber-OKSerialNo-OK").length;
        this.verifyStatus = this.totalData == vData ? "Successed" : "Verification Failed";
