...
@Component({
  selector: 'ngx-factory-fgstock',
  templateUrl: './factory-fgstock.component.html',
  styleUrls: ['./factory-fgstock.component.scss']
})
export class FactoryFGStockComponent implements OnInit {
  ...
  public pageNavigation = "Factory Finish Goods Production"; //"Stock In By OB Or Without PO";
  ...
  private save() {
    ...
    this.StockinwithoutpoService.setFactoryFGStockIn(this.master).subscribe(
      (returns: any) => {
        if (returns.success) {
          if (button == "update") {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      //alert(stockMasterId);
      this.StockinwithoutpoService.GetFactoryFGStockInJSON(stockMasterId).subscribe(
        (data: any) => {
          if (data.success) {
  ...
  private agDelete(event) {
    if (confirm('Are you sure to delete')) {
      ...
      this.StockinService.DeleteFactoryProductionStockIn(this.master.stockMasterId).subscribe(
        (returns: any) => {
          if (returns.success) {
            this.toastrService.success(this.commonService.deletedmsg, "Message");

            //////////////Grid Refresh ///////////////////
            this.StockinwithoutpoService.getStockInWithOtPO(0
            ).subscribe((data: any) => {
              if (data.success) {
                this.rowData = data.data;
              }
  ...
  public addStockdetails() {
    ...
    if (this.master.currentRate == null || this.master.currentRate == undefined || this.master.currentRate == 0) {
      this.toastrService.warning("Please input actual MRP!", "warning");
      return;
    ...
    let detail = {
      ...
      currentRate: this.master.currentRate,
    };
  ...
  public tableHeader = ["#", "Product Name", "Batch No.", "MRP", "Stock Qty.", "Current Stock"];
