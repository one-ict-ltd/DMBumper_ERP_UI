...
@Component({
  selector: 'ngx-factory-fcqc',
  templateUrl: './factory-fcqc.component.html',
  styleUrls: ['./factory-fcqc.component.scss']
})
export class FactoryFCQCComponent implements OnInit {
  ...
  public pageNavigation = "Production QA"; //"Stock In By OB Or Without PO";
  ...
  transectionList: [];
  GETALLTransection() {
    this.StockinwithoutpoService.GetFactoryFGStockInJSONForStock(0).subscribe((retuns: any) => {
      if (retuns.success) {
        this.transectionList = retuns.data.filter(x => x.transactionMasterId == 0).map((val: any) => ({
          id: val.stockMasterId,
          name: val.stockNo + ' | ' + val.stockDate,
        }));
      }
    });
  }

  public getStockDetailsDataLoad(stockMasterId) {
    debugger
    this.master.stockDetailsList = [];
    this.StockinwithoutpoService.getFactoryFGProductionDetailsByIn(
      stockMasterId
    ).subscribe((data: any) => {
      if (data.success) {
  ...
  private save() {
    ...
    if (this.master.transectionSelected == null) {
      this.toastrService.danger("Please select transection.", "Message");
      // this.show = false;
    ...
    this.StockinwithoutpoService.setFactoryFGStockInQA(this.master).subscribe(
      (returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          this.StockinwithoutpoService.GetFactoryFGStockInJSON(0).subscribe(
            (data: any) => {
              if (data.success) {
  ...
  onGridReady(params) {
    ...
    this.StockinwithoutpoService.GetFactoryFGStockInJSON(0).subscribe(
      (data: any) => {
        if (data.success) {
          this.rowData = data.data.filter(x => x.transactionMasterId == 1);
        }
      }
    );
  }
  ...
  public onRowClicked(event) {
    ...
    if (data == "edit") {
      ...
      this.toastrService.info("Can't Delete After QA", "Message");
      //this.agDelete(event);
    } else {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.StockinwithoutpoService.GetStockInWithProductionById(stockMasterId).subscribe(
        (data: any) => {
          if (data.success) {
      ...
      this.StockinService.deleteStockInById(this.master.stockMasterId).subscribe(
        (returns: any) => {
          if (returns.success) {
            ...
            //////////////Grid Refresh ///////////////////
            this.StockinwithoutpoService.GetStockInWithProductionById(
  ...
  public generateStockInReport(stockMasterId) {
    this.StockinwithoutpoService.getRptFactoryFGStockIn(
      stockMasterId
    ).subscribe((returns: any) => {
      if (returns.success) {
