...
@Component({
  selector: 'ngx-rm-pm-direct-stock-in',
  templateUrl: './rm-pm-direct-stock-in.component.html',
  styleUrls: ['./rm-pm-direct-stock-in.component.scss']
})
export class RmPmDirectStockInComponent implements OnInit {
  public company: {
    name: string;
    address: string;
  ...
  public pageNavigation = "RM/PM Direct Stock In";
  ...
  private save() {
    ...
    this.StockinwithoutpoService.saveRmPmStockIn(this.master).subscribe(
      (returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          this.StockinwithoutpoService.getRmPmStockInWithOtPO(0).subscribe(
            (data: any) => {
              if (data.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.StockinwithoutpoService.getRmPmStockInWithOtPO(stockMasterId).subscribe(
        (data: any) => {
          if (data.success) {
      ...
      this.StockinService.deleteStockInById(this.master.stockMasterId).subscribe(
        (returns: any) => {
          if (returns.success) {
            ...
            //////////////Grid Refresh ///////////////////
            this.StockinwithoutpoService.getRmPmStockInWithOtPO(
              0
            ).subscribe((data: any) => {
              if (data.success) {
                this.rowData = data.data;
  ...
  public getProductType() {
    this.productService.getProductTypeForDirectStockIn(1, 1, 0, 'direct-stock-in').subscribe((retuns: any) => {
      if (retuns.success) {
