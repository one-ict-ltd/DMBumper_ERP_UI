...
@Component({
  selector: 'ngx-stock-infrom-production',
  templateUrl: './stock-infrom-production.component.html',
  styleUrls: ['./stock-infrom-production.component.scss']
})
export class StockInfromProductionComponent implements OnInit {
  ...
  GETALLTransection() {
    this.StockinwithoutpoService.GetFactoryFGStockInJSONForStock(0).subscribe((retuns: any) => {
      if (retuns.success) {
        this.transectionList = retuns.data.filter(x => x.transactionMasterId == 1).map((val: any) => ({
    ...
    this.StockinwithoutpoService.saveStockIn(this.master).subscribe(
      (returns: any) => {
        if (returns.success) {
          ...
          this.StockinwithoutpoService.GetStockInWithProductionById(0).subscribe(
            (data: any) => {
              if (data.success) {
      ...
      this.StockinService.deleteStockInById(this.master.stockMasterId).subscribe(
        (returns: any) => {
          if (returns.success) {
            ...
            //////////////Grid Refresh ///////////////////
            this.StockinwithoutpoService.GetStockInWithProductionById(0
            ).subscribe((data: any) => {
              if (data.success) {
                this.rowData = data.data;
              }
