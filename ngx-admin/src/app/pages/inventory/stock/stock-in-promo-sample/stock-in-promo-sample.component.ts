...
@Component({
  selector: 'ngx-stock-in-promo-sample',
  templateUrl: './stock-in-promo-sample.component.html',
  styleUrls: ['./stock-in-promo-sample.component.scss']
})
export class StockInPromoSampleComponent implements OnInit {
  public company: {
    name: string;
    address: string;
  ...
  public pageNavigation = "Receiving Promo/Sample Items";
  ...
  public addStockdetails() {
    if (this.master.productWiseSpecificationId == null || this.master.productWiseSpecificationId === 0 || this.master.productWiseSpecificationId == undefined) {
      this.toastrService.warning("Please select a product!", "warning");
      return;
    }
    if (this.master.stockQty == null || this.master.stockQty <= 0 || this.master.stockQty == undefined) {
      this.toastrService.warning("Please enter receive quantity!", "warning");
      return;
    }
