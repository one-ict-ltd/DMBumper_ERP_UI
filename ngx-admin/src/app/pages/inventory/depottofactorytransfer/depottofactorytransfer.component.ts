...
@Component({
  selector: 'ngx-depottofactorytransfer',
  templateUrl: './depottofactorytransfer.component.html',
  styleUrls: ['./depottofactorytransfer.component.scss']
})
export class DepottofactorytransferComponent implements OnInit {
  ...
  public pageNavigation = "Finish Goods Return (FGR) [Depot To Factory Transfer]";
  public rptHeader = "Finish Goods Return (FGR)";
  ...
  public deleteDetail(index: any) {
    if (confirm('Are you sure to delete?')) {
      this.ProducttransferService
        .DeleteProductTransferDetailsById(
          this.master.lstReqDetailsViewModel[index].productTrnfrDetailsId
        )
        .subscribe((returns: any) => {
          if (returns.success) {
  ...
  generateCrReport(reportFormat: any, stockTransferId: any) {
    ...
    let reportHeaderName = "Finish Goods Return To CWH"; //"Finish Goods Return (FGR)";
