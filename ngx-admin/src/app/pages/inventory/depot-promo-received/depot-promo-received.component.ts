...
@Component({
  selector: 'ngx-depot-promo-received',
  templateUrl: './depot-promo-received.component.html',
  styleUrls: ['./depot-promo-received.component.scss']
})
export class DepotPromoReceivedComponent implements OnInit {
  ...
  constructor(
    ...
    this.columnDefs = [
      ...
      {
        headerName: "Promo Receive No.",
        field: "promoReceivedNo",
        width: 200,
      },
      {
        headerName: "Receive Date",
        field: "depotPromoReceiveDate",
        width: 180,
      },
      {
        headerName: "Purpose",
      ...
      {
        headerName: "Packet Distribution No./Ref.",
  ...
  onGridReady(params) {
    ...
    debugger;
    this.PromoRequisition.GetPromoReceivedById(0).subscribe(
      (data: any) => {
        if (data.success) {
  ...
  public pageNavigation = "Packet Received By Depot";
  ...
  private save() {
    ...
    this.master.lstDetailsViewModel = this.master.lstDetailsViewModel.map(detail => ({
      promoReceiveDetailsId: detail.promoReceiveDetailsId,
    ...
    if (this.master.depotPromoReceiveId != 0) {
      debugger;
      this.master.promoTrnfId = this.master.depotPromoReceiveId;
    }
    this.show = true;
    ...
    this.PromoRequisition.SaveDepotPromoReceive(this.master).subscribe(
      (returns: any) => {
        if (returns.success) {
          if (button == "update") {
  ...
  private agDelete(event) {
    if (confirm('Are sure to delete?')) {
      let promoTrnfrId = event.node.data.depotPromoReceiveId;;
      this.PromoRequisition.DeletePromoReceiveById(
        promoTrnfrId
        ...
        if (returns.success) {
          ...
          this.toastrService.warning("This packet has already been distributed to territory, you can not delete it.", "Message");
        }
      });
    }
  }
  ...
  public GetAllPacketByDistribution(event: any) {
    ...
    this.PromoRequisition.GetAllPacketByDistribution(selectedDistributionId).subscribe((returns: any) => {
      if (returns.success) {
