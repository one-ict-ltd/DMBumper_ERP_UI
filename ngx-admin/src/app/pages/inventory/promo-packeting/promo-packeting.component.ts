...
@Component({
  selector: 'ngx-promo-packeting',
  templateUrl: './promo-packeting.component.html',
  styleUrls: ['./promo-packeting.component.scss']
})
export class PromoPacketingComponent implements OnInit {
  serverDate: any[];
  ...
  public pageNavigation = "Promo Packeting";
  ...
  private save() {
    ...
    this.PromoRequisition.SavePromoPacketing(this.master).subscribe(
      (returns: any) => {
        if (returns.success) {
          if (button == "update") {
  ...
  // getTotalOfTable(listOfdata : any) {
  //   debugger;
  //   this.totalRequQty = 0;
  ...
  //     this.totalNumberOfPacks += Number(this.master.lstDetailsViewModel[i].transferQty);
  //     this.totalRequQty += Number(this.master.lstDetailsViewModel[i].quantity);
  //     this.totalPackedQty += Number(this.master.lstDetailsViewModel[i].transferedQty);
  //     this.totalRestQty += Number(this.master.lstDetailsViewModel[i].restQty);
  //   }
