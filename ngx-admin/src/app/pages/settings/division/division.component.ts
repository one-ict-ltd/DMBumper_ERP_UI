...
export class DivisionComponent implements OnInit {
  ...
  public pageNavigation = "Division";
  ...
  private save() {
    if (this.master.divisionName == "" || this.master.countryId ==0) {
      ...
      this.hrmmasterService.saveDivision(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.hrmmasterService.getDivision(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    this.master.divisionsId = event.node.data.divisionsId;
    this.hrmmasterService.deleteDivision(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.hrmmasterService.getDivision(this.master.divisionsId).subscribe((data: any) => {
        if (data.success) {
          this.master = data.data[0];
          this.getCountry();

          this.master.CountrySelected = {
  ...
  public CountryList = [];
  public getCountry() {
    this.hrmmasterService.GetCountry().subscribe((retuns: any) => {
      if (retuns.success) {
        this.CountryList = retuns.data.map((val: any) => ({
          id: val.countryId,
