...
export class DistrictComponent implements OnInit {
  ...
  public pageNavigation = "District";
  ...
  private save() {
    if (this.master.districtName == "" || this.master.divisionsId ==0) {
      ...
      this.hrmmasterService.saveDistrict(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.hrmmasterService.getDistrict(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    this.master.districtsId = event.node.data.districtsId;
    this.hrmmasterService.deleteDistrict(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.hrmmasterService.getDistrict(this.master.districtsId).subscribe((data: any) => {
        if (data.success) {
          this.master = data.data[0];
          this.getDropDownDivision();
          this.master.divisionSelected = {
            id: data.data[0].divisionsId,
  ...
  public DivisionList = [];
  public getDropDownDivision() {
    this.hrmmasterService.GetDropDownDivision().subscribe((retuns: any) => {
      if (retuns.success) {
        this.DivisionList = retuns.data.map((val: any) => ({
          id: val.divisionsId,
