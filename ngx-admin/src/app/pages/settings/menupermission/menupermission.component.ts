...
export class MenupermissionComponent implements OnInit {
  ...
  public pageNavigation = "Menu Permission";
  ...
  public getModulePermisssion() {
    //debugger;
    this.master.moduleSelected = null;
    this.moduleItems = null;
    this.modulepermissionService.getModulePermissionById(this.master.companyId, 0).subscribe((returns: any) => {
  ...
  private getMenu() {
    this.comboService.getDefaultMenu(this.master.moduleId).subscribe((data: any) => {
      if (data.success) {
        //debugger;
  ...
  private save() {
    ...
    this.menupermissionService.saveMenuPermission(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
