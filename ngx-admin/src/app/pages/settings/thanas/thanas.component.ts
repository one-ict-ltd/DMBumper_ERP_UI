...
@Component({
   ...
   styleUrls: ['./thanas.component.scss']
})
export class ThanasComponent implements OnInit {
  ...
  private save() {
    if (this.master.thanaName == "" || this.master.districtsId ==0) {
      ...
      this.hrmmasterService.saveThanas(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
          ...
          //////////////Grid Refresh ///////////////////
          this.getMaster();
          this.hrmmasterService.getThanas(0).subscribe((data: any) => {
            if (data.success) {
  ...
  private agDelete(event) {
    this.master.thanasId = event.node.data.thanasId;
    this.hrmmasterService.deleteThanas(this.master).subscribe((returns: any) => {
      if (returns.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.hrmmasterService.getThanas(this.master.thanasId).subscribe((data: any) => {
        if (data.success) {
          this.master = data.data[0];
          this.getDropDownDistrict();
          this.master.districtSelected = {
            id: data.data[0].districtsId,
