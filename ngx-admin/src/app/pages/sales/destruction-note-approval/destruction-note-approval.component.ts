...
@Component({
  selector: 'ngx-destruction-note-approval',
  templateUrl: './destruction-note-approval.component.html',
  styleUrls: ['./destruction-note-approval.component.scss']
})
export class DestructionNoteApprovalComponent implements OnInit {
  ...
  public pageNavigation = "Destruction Note Approval";//"Damage Expire Products Return";
  ...
  private save() {
    debugger
    if (this.master.approvalStatusValue == null || this.master.approvalStatusValue == 0) {
      this.toastrService.danger("Please Select a approval Type", "Message");
      return false;
    }
    ...
    this.salesinvoiceService
      .DestructionNoteApproval(this.master)
  ...
  private agReport(event) {
    ...
    this.getReportData(event.data.damageExpireProductReturnMasterId, event.data.MarketOrDepo);
  ...
  loadGridData(isApproved: number) {
    debugger
    this.salesinvoiceService
      .GetAllDamageExpireProductReturn('', isApproved).subscribe((data: any) => {
        if (data.success) {
          console.log(data);
          if (isApproved == 1)
  ...
  loadApprovalStatusList() {
    ...
    this.master.approvalStatusValue = this.ApprovalStatusSelected['id'];
  }
  private getReportData(masterId, MarketOrDepo) {
    debugger
    let userInfo = this.commonService.GetUserProfileJson();
    this.apiUrl = `SalesInvoiceReport/GetDamageOrExpireReportById?reportFormat=Pdf&userId=${userInfo[0].employeeid}&masterId=${masterId}&MarketOrDepo=${MarketOrDepo}`;
    debugger;
    this.commonService.GetCrystalReportData(this.apiUrl).subscribe((returns: any) => {
      let res = JSON.parse(returns);
      if (res.status) {
  ...
  //   this.bodyData = [];
  //   debugger;
  //   this.apiUrl = `SalesInvoice/GetAllDamageExpireReturnByIdJson?masterId=${collectionId}`;
