...
@Component({
  selector: "ngx-sales-distribution-approval",
  templateUrl: "./sales-distribution-approval.component.html",
  styleUrls: ["./sales-distribution-approval.component.scss"],
})
export class SalesDistributionApprovalComponent implements OnInit {
  protected options: {};
  ...
  public pageNavigation = "Sales Distribution Approval";
  public buttons = this.commonService.btnList;

  public ButtonAction() {
    if (this.commonService.buttonClicked == "create") {
      this.GetSalesDistributionListForApproval();
      this.getMaster();
      //this.show = false;
  ...
  private save() {
    ...
    if (
      ...
      this.toastrService.danger("Please select a Approval Status.", "Message");
      this.commonService.valueSet("create");
      return false;
    }

    // if (
    //   this.master.lstMasterViewModel.length == 0 ||
    //   this.master.lstMasterViewModel == null
    // ) {
    //   this.toastrService.danger("Please enter product.", "Message");
    //   this.commonService.valueSet("create");
    //   return false;
    // }
    this.show = true;
    console.log(this.master);
    this.SalesDistributionService.ApproveSalesDistribution(
      this.master
      ...
      if (returns.success) {
        ...
        this.SalesDistributionService.GetSalesDistributionListForApproval(
          0
        ).subscribe((data: any) => {
          if (data.success) {
  ...
  onGridReady(params) {
    ...
    // this.SalesDistributionService.GetSalesDistributionApprovedList(0).subscribe(
    //   (data: any) => {
    //     if (data.success) {
    //       this.rowData = data.data;
    //     }
    ...
    if (temp === 0) {
      ...
      this.SalesDistributionService.GetSalesDistributionListForApproval(
        ...
        if (data.success) {
          ...
          this.master.lstMasterViewModel = data.data[0];
        }
      });
      this.ngOnInit();
    }
  ...
  GetSalesDistributionListForApproval() {
    this.commonService.valueSet("create");
    this.SalesDistributionService.GetSalesDistributionListForApproval(
      0
    ).subscribe((returns: any) => {
  ...
  public calculateTotal(index: any) {
    // let totalPrice = 0;
    // let invoiceQty = this.master.lstMasterViewModel[index].invoiceQty == "" ? 0 : this.master.lstMasterViewModel[index].invoiceQty;
    // let price = this.master.lstMasterViewModel[index].price == "" ? 0 : this.master.lstMasterViewModel[index].price;
    // let vat = this.master.lstMasterViewModel[index].vat == "" ? 0 : this.master.lstMasterViewModel[index].vat;
    // let ait = this.master.lstMasterViewModel[index].ait == "" ? 0 : this.master.lstMasterViewModel[index].ait;
    // let discountAmount = this.master.lstMasterViewModel[index].discountAmount == "" ? 0 : this.master.lstMasterViewModel[index].discountAmount;
    // totalPrice = (invoiceQty * price);
    // vat = (totalPrice * (vat / 100));
    // ait = (totalPrice * (ait / 100));
    // discountAmount = (totalPrice * (discountAmount / 100));
    // this.master.lstMasterViewModel[index].total = (totalPrice + vat + ait) - discountAmount;
    // this.calculateGrandTotal();
  }

  calculateGrandTotal() {
    // let totalGross = 0;
    // this.master.lstMasterViewModel.forEach(row => {
  ...
  public addToDetailsGrid() {
    ...
    this.master.lstMasterViewModel.push(elements);
    this.calculateGrandTotal();
  }
  ...
  public deleteDetails(index: any) {
    this.SalesDistributionService.DeleteSalesDistributionDetailsById(
      this.master.lstMasterViewModel[index].salesInvDetailsId
    ).subscribe((returns: any) => {
      if (returns.success) {
        this.toastrService.success(this.commonService.deletedmsg, "Message");
      }
    ...
    this.selectedRow = this.master.lstMasterViewModel[index];
    this.master.lstMasterViewModel.splice(index, 1);
