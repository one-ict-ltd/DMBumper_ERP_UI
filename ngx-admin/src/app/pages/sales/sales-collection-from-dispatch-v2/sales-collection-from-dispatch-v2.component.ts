...
@Component({
  selector: 'ngx-sales-collection-from-dispatch-v2',
  templateUrl: './sales-collection-from-dispatch-v2.component.html',
  styleUrls: ['./sales-collection-from-dispatch-v2.component.scss']
})
export class SalesCollectionFromDispatchV2Component implements OnInit {
  ...
  show: boolean = true;
  // Bill-collection page: hide territory/MIO/invoice-type/depot/officer fields (not used for tenders).
  hideTerritoryFields: boolean = true;
  // Bill-collection page: money receipt not used for tenders — hide it + skip its validations.
  hideMoneyReceipt: boolean = true;
  ...
  public ButtonAction() {
    if (this.commonService.buttonClicked == "create") {
      ...
      this.LoadBillCustomers();   // bill-collection: load customers on open (no territory)
  ...
  public getMaster() {
    ...
    //this.GetSalesInvoiceListfromDispatch();
    //this.ChangeNumber();
    this.billcollectionService.GetMaxSalesCollectionNumberForBill(this.commonService.DateFormat(this.master.collectionDate)).subscribe((retuns: any) => {
      if (retuns.success) {
    ...
    this.GetCollectionAmountWiseCommissionPercent();
  }

  partyList = [];
  // Bill-collection: customers who have a bill-sourced invoice (no territory).
  LoadBillCustomers() {
    this.partyList = [];
    this.master.partyId = 0;
    this.master.partySelected = null;
    // Money receipt is hidden for tenders — no need to load it.
  ...
  public GetAllPendingMoneyRecipts() {
    ...
    this.MoneyReceiptDetails = "";
    this.master.moneyReceiptId = 0;
    this.master.moneyReceiptNo = '';
    this.master.collectionAmountInput = 0;
    this.master.moneyReceiptSelected = null;
    this.master.moneyReceiptSelected = null;

    // Bill-collection: no territory — load recent pending receipts (capped at 200 by the SP)
    this.billcollectionService.GetAllPendingMoneyReciptsForBill().subscribe((retuns: any) => {
      if (retuns.success) {
        if (retuns.data.length == 0) this.toastrService.info("No money receipt found !", 'Info');
  ...
  changeMoneyRecipt(event: any) {
    ...
    if (this.master.moneyReceiptSelected != null) {
  ...
  ValidationForSave(): boolean {
    ...
    // Tender bill-collection: money receipt is not used here, so skip its validations
    // (MR-No required, MR-select, and "input must equal collection"). The amount the
    // collector types per invoice IS the collection.
    if (!this.hideMoneyReceipt) {
      if (this.master.moneyReceiptNo.trim() == "") {
        this.toastrService.warning("Money Receipt No. is empty.", "Message");
        return false;
      }
      else if (this.master.moneyReceiptSelected == null) {
        this.toastrService.warning("Select a Money Receipt No.", "Message");
      }
      else if (this.master.collectionAmountInput != (this.master.collectionAmount)) {
        this.toastrService.warning("Collection Amount & Total Collection Amount must be equal!", "Message");
        return false;
      }
      else if (this.master.collectionAmountInput == null || this.master.collectionAmountInput <= 0) {
        this.toastrService.warning("Total Collection input amount must be greater than zero!", "Message");
        return false;
      }
    }

    if (this.master.paymentModeId == null || this.master.paymentModeId == 0) {
      this.toastrService.warning("Please Select a Payment Mode!", "Message");
      return false;
    }
    if (this.checkedChildCount == 0) {
      this.toastrService.warning("Minimum one collection required!", "Message");
      return false;
    }
    if (this.master.collectionAmount == 0 && this.master.collectionMasterId == 0) {
  ...
  private save() {
    ...
    // Tender: no money receipt, so the collected total IS the input amount.
    if (this.hideMoneyReceipt) {
    ...
    this.salesinvoiceService
      .SaveCollectionInvoiceFormDispatch_v2ForBill(this.master)
      .subscribe((returns: any) => {
        if (returns.success) {
    ...
    if (confirm(`Are you sure to delete this [ ${event.data.collectionNumber} ]?`)) {
      ...
      this.billcollectionService.DeleteSalesCollectionByMasterId(collectionMasterId).subscribe((returns: any) => {
        if (returns.success) {
          ...
          //this.toastrService.warning("Access dennied! You can not delete this collection.", "Message");
          this.toastrService.warning(returns.message, "Message");
        }
      });
    }
  ...
  CalculateSummary() {
    ...
    for (let i = 0; i < this.master.lstDetailsViewModel.length; i++) {
      const element = this.master.lstDetailsViewModel[i];
      ...
      if (this.master.lstDetailsViewModel.length == i + 1) {
        ...
        */
        ...
        this.RestAmount = this.master.collectionAmountInput - this.master.collectionAmount;
      }
      ...
      //ttlDues += element.balanceAmount ?? 0;
      ttlDues += element.dueAmount ?? 0;
      //console.log('ttlDues: ', ttlDues)
    }
    //);
    this.master.dues = ttlDues;//added on: 2024-05-29 
  }
  ...
  DiscountPercentStatus: string = "";
  GetDiscountPercentMsg(Percent: number = 0, InvoiceNo: string, collAmount: number = 0, rowIndex: number = -1) {
    this.DiscountPercentStatus = "";
    if (InvoiceNo != "" && rowIndex > -1) {
      let bonusDiscount = this.master.lstDetailsViewModel[rowIndex].bonusDiscount ?? 0;
      this.DiscountPercentStatus = InvoiceNo == "" ? "" : `${Percent}% discounts (${bonusDiscount} TK) apply for TK.${collAmount} (without VAT) collection; ${InvoiceNo}`;
    }
  }
  ...
  CalculateBonusDiscountIn(rowIndex: number, flag: any = "Auto") {
    ...
    // Tender: typing an amount auto-selects the row (no separate Select tick needed)
    if (this.hideMoneyReceipt) {
      this.master.lstDetailsViewModel[rowIndex].isSelect = (inputamount > 0) ? 1 : 0;
    }
    ...
    if (flag == "Auto") {
      ...
      let percentValue: number = this.master.lstDetailsViewModel[rowIndex].percentValue;
      console.log("CalculateBonusDiscountIn percentValue: ", percentValue);
      console.log("CalculateBonusDiscountIn collAmont: ", collAmont);
      console.log("CalculateBonusDiscountIn vatAdjustment: ", this.master.lstDetailsViewModel[rowIndex].vatAdjustment);
      ...
      if (this._CompanyId == 2) {
        ...
        /* new Percentage Policy add for AH*/

        let discountUpToDays = this.master.lstDetailsViewModel[rowIndex].discountUpToDays ?? 0;
        let collectionAmount = this.master.lstDetailsViewModel[rowIndex].collectionAmount ?? 0;

        console.log("discountUpToDays: ", discountUpToDays);
        console.log("collectionAmount: ", collectionAmount);

        let currentPercentValue = this.percentValueList.filter((el) => ((el.cDaysFrom <= discountUpToDays && el.cDaysTo >= discountUpToDays) && (el.cAmountFrom <= collectionAmount && el.cAmountTo >= collectionAmount)));

        //console.log("currentPercentValue: ", currentPercentValue);

        if (currentPercentValue != null && currentPercentValue.length > 0 && this.master.lstDetailsViewModel[rowIndex].isFlatRateInv == 0) {
          this.master.lstDetailsViewModel[rowIndex].percentValue = currentPercentValue[0].commissionPercent;
        }
        else {
          this.master.lstDetailsViewModel[rowIndex].percentValue = 0;
        }
        console.log("currentPercentValue: ", currentPercentValue);
      }

      this.GetDiscountPercentMsg(this.master.lstDetailsViewModel[rowIndex].percentValue, this.master.lstDetailsViewModel[rowIndex].salesInvoiceNo);
      ...
      console.log("selected Row: ", this.master.lstDetailsViewModel[rowIndex]);

      this.GetDiscountPercentMsg(this.master.lstDetailsViewModel[rowIndex].percentValue, this.master.lstDetailsViewModel[rowIndex].salesInvoiceNo, totaLWOVAT, rowIndex);
    }
    else {
      // do something here if needed.
    }
  ...
  checkChange(e, rowIndex: number) {
    ...
    if (e.target.checked) {
      ...
      if (this._CompanyId == 2) {
        ...
        let discountUpToDays = this.master.lstDetailsViewModel[rowIndex].discountUpToDays ?? 0;
        let collectionAmount = totaLWOVAT; //this.master.lstDetailsViewModel[rowIndex].collectionAmount ?? 0;
        ...
        console.log("objCurrentPercentValue: ", currentPercentValue);
        console.log("collectionAmount: ", collectionAmount);

      }

      this.GetDiscountPercentMsg(this.master.lstDetailsViewModel[rowIndex].percentValue, this.master.lstDetailsViewModel[rowIndex].salesInvoiceNo, totaLWOVAT);

      //console.log("collectionAmount: ", collectionAmount);
      console.log("percentValue: ", this.master.lstDetailsViewModel[rowIndex].percentValue);

      console.log("totaLWOVAT: ", totaLWOVAT);
      console.log("bonusDiscount: ", Math.round(totaLWOVAT * this.master.lstDetailsViewModel[rowIndex].percentValue * 0.01));
      //debugger;
      ...
      this.GetDiscountPercentMsg(0, "");
    }


    this.checkIncentiveAmt = this.master.lstDetailsViewModel[rowIndex].incentiveAmount ?? 0;
    this.checkBonusDiscountAmt = this.master.lstDetailsViewModel[rowIndex].bonusDiscount ?? 0;
    this.checkCollectionAmt = this.master.lstDetailsViewModel[rowIndex].collectionAmount ?? 0;

    this.master.lstDetailsViewModel[rowIndex].inputAmount = e.target.checked ? Math.round(this.master.lstDetailsViewModel[rowIndex].collectionAmount
    ...
    console.log((e.target.checked ? 'Checked Row: ' : 'UnChecked Row: '), this.master.lstDetailsViewModel[rowIndex]);
  ...
  GetSalesInvoiceListfromDispatch() {
    ...
    this.DiscountPercentStatus = "";
    this.master.lstDetailsViewModel = [];
    if (this.master.partyId == 0) return;   // bill-collection: driven by customer, not territory

    this.salesinvoiceService
      .GetSalesInvoiceListForBillCollection(this.master.collectionMasterId, this.master.partyId, this.commonService.DateFormat(this.master.collectionDate))
      .subscribe((returns: any) => {
        if (returns.success) {
          this.master.lstDetailsViewModel = returns.data;
          this.invoiceCount = returns.data.length;
          if (this.invoiceCount == 0) this.toastrService.info("No invoice found for collection !", 'Info');
          this.CalculateSummary();
        }
      });
  }
  ...
  percentValueList = [];
  GetCollectionAmountWiseCommissionPercent() {
    //debugger;
    this.master.lstDetailsViewModel = [];
    this.salesinvoiceService
      .GetCollectionAmountWiseCommissionPercent(this.commonService.DateFormat(this.master.collectionDate))
      .subscribe((returns: any) => {
        if (returns.success) {
          //console.log(returns.data);
          this.percentValueList = returns.data;
          console.log('percentValueList: ', this.percentValueList);
        }
      });
    this.GetDiscountPercentMsg(0, "");
  }
