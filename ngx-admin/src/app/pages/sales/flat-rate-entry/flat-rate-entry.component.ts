...
@Component({
  selector: 'ngx-flat-rate-entry',
  templateUrl: './flat-rate-entry.component.html',
  styleUrls: ['./flat-rate-entry.component.scss']
})
export class FlatRateEntryComponent implements OnInit {

  public pageNavigation = "Flat Rate Entry";
  ...
  loadToDateShow: Date = new Date(this.loadFromDateShow.getFullYear(), 12, 0);
  ...
  LoadGridData() {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      ...
      let toDate = this.commonService.DateFormat(this.loadToDateShow);
      this.PolicyService.GetSalesDiscountRatePolicy(0, this.depotCode, 0, "Flat", fromDate, toDate).subscribe((data: any) => {
        if (data.success) {
  ...
  private save() {
    ...
    this.PolicyService.SaveFlatRatePolicyList(this.listOfItems).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
  ...
  public calculateAmount() {
    ...
    if (this.master.discountType == "Percentage" || this.master.discountType == "Customer Discount") {
      ...
      // this.master.amount = this.roundToDigit((this.master.flatAmount - this.unitVat), 2);
      // this.master.discountAmount = this.roundToDigit((this.master.price - this.master.amount), 2)
    }

    this.master.amount = 0;
    this.master.discountAmount = 0;

    if (this.master.flatAmount > 0) {
      this.master.percentAmount = 0;
      this.master.amount = this.roundToDigit((this.master.flatAmount - this.unitVat), 2);
      this.master.discountAmount = this.roundToDigit((this.master.price - this.master.flatAmount), 2);
    }
  }
  ...
  public partiesSelected: any = null;
  ...
  setDepoCode() {
    debugger
    if (this.depotSelected == (null || undefined)) {
      this.depotCode = "";
    }
    else this.depotCode = this.depotSelected.id;
  }
  addToDetailsGrid(master: any) {
    if (this.ValidationForRowItemAdd(master)) {
      ...
      this.showbody = true;
      let obj = {
        DiscountRateId: master.DiscountRateId,
        productWiseSpecificationId: master.productWiseSpecificationId,
        ...
        productName: this.productSpecListSelected,
        discountTypeSelected: master.discountTypeSelected,
        flatAmount: master.flatAmount,
        isAll: true,
        newPrice: `Flat Rate: ${this.master.flatAmount}  = TP:${master.amount}  VAT: ${this.unitVat}`
      };
      this.listOfItems.splice(0, 0, obj);
      obj = null;
      this.productSpecListSelected = null;
      this.master.price = 0;
      this.master.flatAmount = null;
      this.master.discountAmount = 0;
      this.master.amount = 0;
      this.unitVat = 0;
      //this.getMaster();
  ...
  ValidationForRowItemAdd(addableItem: any): boolean {
    debugger
    if (addableItem.productWiseSpecificationId == (undefined || null || 0)) {
      this.toastrService.warning("Please select a product", "Warning");
      return false;
    } if (this.master.flatAmount <= 0) {
      this.toastrService.warning("Please enter Flat Rate ", "Warning");
      return false;
    } if (this.commonService.DateFormat(addableItem.fromDate) == (null || undefined)) {
      this.toastrService.warning("Please enter start date", "Warning");
      return false;
    } if (this.commonService.DateFormat(addableItem.endDate) == (null || undefined)) {
      this.toastrService.warning("Please enter end date", "Warning");
      return false;
    } if (this.commonService.DateFormat(addableItem.fromDate) > this.commonService.DateFormat(addableItem.endDate)) {
      ...
      if (this.isDuplicate(addableItem)) {
  ...
  isDuplicate(rowData: any): boolean {
    return this.listOfItems.some(element =>
      rowData.productWiseSpecificationId === element.productWiseSpecificationId &&
