...
@Component({
  selector: 'ngx-miscellaneous-report',
  templateUrl: './miscellaneous-report.component.html',
  styleUrls: ['./miscellaneous-report.component.scss']
})
export class MiscellaneousReportComponent implements OnInit {
  ...
  pageNavigation = "Miscellaneous Report";
  ...
  generateCrReport(reportFormat: any) {
    ...
    this.apiUrl = `SalesInvoiceReport/GetMiscellaneousReportDepotByType?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&miscTypeId=${miscTypeId}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&depotCode=${depotCode}&productId=${productId}`;
