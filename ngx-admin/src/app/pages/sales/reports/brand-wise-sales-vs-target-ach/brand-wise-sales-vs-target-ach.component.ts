...
@Component({
  selector: 'ngx-brand-wise-sales-vs-target-ach',
  templateUrl: './brand-wise-sales-vs-target-ach.component.html',
  styleUrls: ['./brand-wise-sales-vs-target-ach.component.scss']
})
export class BrandWiseSalesVsTargetAchComponent implements OnInit {
  ...
  pageNavigation = "Brand Wise Sales Target vs Achievement (AH)";
  ...
  generateCrReport(reportFormat: any) {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      ...
      this.apiUrl = `SalesInvoiceReport/GetBrandWiseSalesTargetVsAchievementReport?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&zoneCode=${this.zoneCode}&regionCode=${this.regionCode}&areaCode=${this.areaCode}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&empCode=${this.mioCode}`;
  ...
  private getReportData() {
    ...
    let userInfo = this.commonService.GetUserProfileJson();
    this.apiUrl = `SalesInvoiceReport/GetBrandWiseSalesTargetVsAchievementReport?reportFormat=Pdf&userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&zoneCode=${this.zoneCode}&regionCode=${this.regionCode}&areaCode=${this.areaCode}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&empCode=${this.mioCode}`;
    debugger;
    this.commonService.GetCrystalReportData(this.apiUrl).subscribe((returns: any) => {
      let res = JSON.parse(returns);
      //console.log(res);
