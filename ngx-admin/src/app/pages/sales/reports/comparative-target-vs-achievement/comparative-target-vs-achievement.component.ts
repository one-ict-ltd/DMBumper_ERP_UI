...
@Component({
  selector: 'ngx-comparative-target-vs-achievement',
  templateUrl: './comparative-target-vs-achievement.component.html',
  styleUrls: ['./comparative-target-vs-achievement.component.scss']
})
export class ComparativeTargetVsAchievementComponent implements OnInit {
  ...
  getReportData(): void {
    ...
    const apiUrl = `SalesInvoice/GetTargetVsAchievementComparativeReport?depotCode=${params.depotCode}&territoryCode=${params.territoryCode}&fromDate=${this.commonService.DateFormat(params.fromDate)}&toDate=${this.commonService.DateFormat(params.toDate)}`;
    this.commonService.getReportData(apiUrl).pipe(take(1)).subscribe((returns: any) => {
      if (returns.success) {
        this.rowData = returns.data;
        console.log(this.datePipe.transform(params.toDate, 'yy'));
  ...
  generateCrReport(reportFormat: any) {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      ...
      const apiUrl = `SalesInvoiceReport/GetTargetVsAchievementComparativeReport?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&fDate=${params.fromDate}&tDate=${params.toDate}`;
