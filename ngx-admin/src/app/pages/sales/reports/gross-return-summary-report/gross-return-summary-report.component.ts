...
@Component({
  selector: 'ngx-gross-return-summary-report',
  templateUrl: './gross-return-summary-report.component.html',
  styleUrls: ['./gross-return-summary-report.component.scss']
})
export class GrossReturnSummaryReportComponent implements OnInit {
  ...
  pageNavigation = "Gross Return Summary Report";
  ...
  generateCrReport(reportFormat: any) {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      ...
      this.apiUrl = `SalesInvoiceReport/GetSalesGrossReturnSummary?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&masterId=${0}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&depotCode=${depotCode}&territoryCode=${territoryCode}&partyId=${partyId}`;
    ...
    this.salesinvoiceService
      ...
      .subscribe((returns: any) => {
        if (returns.success) {
          ...
          this.lstInvoiceDetails = returns.data;//.filter(
