...
@Component({
  selector: 'ngx-depot-wise-discount-summery',
  templateUrl: './depot-wise-discount-summery.component.html',
  styleUrls: ['./depot-wise-discount-summery.component.scss']
})
export class DepotWiseDiscountSummeryComponent implements OnInit {
  ...
  pageNavigation = "Depot Wise Discount Summery";
  ...
  generateCrReport(reportFormat: any) {
    //debugger;
    if (!this.depotCode) {
      this.toastrService.warning('please select a depot.', 'Warning');
      return;
    }
    if (!this.selectedDiscountType) {
      this.toastrService.warning('please select a discount type.', 'Warning');
      return;
    }
    this.apiUrl = "";
    ...
    this.partyId = this.partySelected?.id || 0;
    this.apiUrl = `SalesInvoiceReport/GetDepoWiseDiscountSummeryReport?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&partyId=${this.partyId}&discountType=${this.selectedDiscountType}`;
  ...
  getAllTerritory(depotCode: any = '') {
    ...
    // this.GetPartyByTerritoryCode(this.territoryCode);
  }
