...
@Component({
  selector: 'ngx-product-wise-discount-report',
  templateUrl: './product-wise-discount-report.component.html',
  styleUrls: ['./product-wise-discount-report.component.scss']
})
export class ProductWiseDiscountReportComponent implements OnInit {
  ...
  pageNavigation = "Product Wise Discount Report";
  ...
  generateCrReport(reportFormat: any) {
    //debugger;
    if (!this.productWiseSpecificationId) {
      this.toastrService.warning('please select a product.', 'Warning');
      return;
    }
    this.apiUrl = "";
    let userInfo = this.commonService.GetUserProfileJson();
    this.depotCode = this.depotSelected?.id || '';
    this.apiUrl = `SalesInvoiceReport/GetProductWiseDiscountReport?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&productWiseSpecificationId=${this.productspecificationSelected?.id}&depotCode=${this.depotCode}&discountType=${this.selectedDiscountType}`;
  ...
  productspecificationSelected: any;
