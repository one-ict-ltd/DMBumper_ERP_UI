...
@Component({
  selector: 'ngx-new-category-wise-sales-vs-target-ach',
  templateUrl: './new-category-wise-sales-vs-target-ach.component.html',
  styleUrls: ['./new-category-wise-sales-vs-target-ach.component.scss']
})
export class NewCategoryWiseSalesVsTargetAchComponent implements OnInit {
  ...
  pageNavigation = "New Category Wise Sales Target vs Achievement (AH)";
  ...
  generateCrReport(reportFormat: any) {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      ...
      this.apiUrl = `SalesInvoiceReport/GetNewCategoryWiseSalesTargetVsAchievementReport?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&zoneCode=${this.zoneCode}&regionCode=${this.regionCode}&areaCode=${this.areaCode}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&empCode=${this.mioCode}`;
  ...
  private getReportData() {
    ...
    let userInfo = this.commonService.GetUserProfileJson();
    this.apiUrl = `SalesInvoiceReport/GetNewCategoryWiseSalesTargetVsAchievementReport?reportFormat=Pdf&userId=${userInfo[0].employeeid}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&zoneCode=${this.zoneCode}&regionCode=${this.regionCode}&areaCode=${this.areaCode}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&empCode=${this.mioCode}`;
    debugger;
    this.commonService.GetCrystalReportData(this.apiUrl).subscribe((returns: any) => {
      let res = JSON.parse(returns);
      //console.log(res);
