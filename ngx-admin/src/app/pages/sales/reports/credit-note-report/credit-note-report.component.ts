...
@Component({
  selector: 'ngx-credit-note-report',
...
export class CreditNoteReportComponent implements OnInit {
  ...
  pageNavigation = "Market Expire Report";
  ...
  generateCrReport(reportFormat: any) {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      ...
      this.apiUrl = `SalesInvoiceReport/GetCreditNoteReport?reportFormat=Pdf&userId=${userInfo[0].employeeid}&masterId=${0}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&depotCode=${depotCode}&territoryCode=${territoryCode}&partyId=${partyId}`;
