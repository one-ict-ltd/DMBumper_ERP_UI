...
@Component({
  selector: 'ngx-sales-register-report',
...
export class SalesRegisterReportComponent implements OnInit {
  ...
  pageNavigation = "Sales Register Report";
  ...
  private getReportData() {
    ...
    let userInfo = this.commonService.GetUserProfileJson();
    //console.log("userInfo[0].employeeid", userInfo[0].employeeid);
    this.apiUrl = `SalesInvoiceReport/GetSalesRegisterReport?reportFormat=Pdf&userId=${userInfo[0].employeeid}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&partyId=${this.partyId}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}&zoneCode=${this.zoneCode}&regionCode=${this.regionCode}&areaCode=${this.areaCode}&typeId=${this.typeId}`;
