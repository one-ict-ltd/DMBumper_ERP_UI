...
@Component({
  selector: 'ngx-collection-bonus-report',
  templateUrl: './collection-bonus-report.component.html',
  styleUrls: ['./collection-bonus-report.component.scss']
})
export class CollectionBonusReportComponent implements OnInit {
  ...
  pageNavigation = "Collection Invoice Discount Percent Report";
  ...
  generateCrReport(reportFormat: any) {
    debugger;
    const fromDate = this.tDate;
    const toDate = this.fDate;
    if (this.commonService.validateDates(fromDate, toDate)) {
    this.apiUrl = "";
    let userInfo = this.commonService.GetUserProfileJson();
    console.log("userInfo[0].employeeid", userInfo[0].employeeid);
    this.apiUrl = `SalesInvoiceReport/GetCollectionBonusReport?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}`;
  ...
  private getReportData() {
    debugger;
    this.apiUrl = "";
    this.apiUrl = `SalesCollection/GetCollectionBonusReport?depotCode=${this.depotCode}&territoryCode=${this.territoryCode}&fDate=${this.commonService.DateFormat(this.fDate)}&tDate=${this.commonService.DateFormat(this.tDate)}`;
    ...
    this.commonService.getReportData(this.apiUrl).subscribe((returns: any) => {
      ...
      this.bodyData.forEach(a => {
        ...
        this.totalNetSalesAmt += (a.ttlSalesAmount);
        this.totalNetCollectionAmt += (a.ttlCollectionAmount);
