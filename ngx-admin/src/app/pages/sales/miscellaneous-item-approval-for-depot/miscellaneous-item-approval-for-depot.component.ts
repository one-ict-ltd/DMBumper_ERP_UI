...
@Component({
  selector: 'ngx-miscellaneous-item-approval-for-depot',
  templateUrl: './miscellaneous-item-approval-for-depot.component.html',
  styleUrls: ['./miscellaneous-item-approval-for-depot.component.scss']
})
export class MiscellaneousItemApprovalForDepotComponent implements OnInit {
  ...
  public pageNavigation = "Miscellaneous Item List For Approval(Depot)";//"Damage Expire Products Return";
  ...
  SaveValidation(): boolean {
    ...
    if (count == 0) {
      this.toastrService.danger("Please select a miscellaneous item for approval.","Message");
      return false;
    }
    else{
      return true;
  ...
  private save() {
    ...
    this.miscellaneousItemService
      .SaveMiscellaneousItemForDepotApproval(this.master)
  ...
  loadGridData(isApproved: number) {
    debugger
    this.miscellaneousItemService
      .GetALLMiscellaneousItemDepotByApproval(isApproved).subscribe((data: any) => {
        if (data.success) {
          console.log(data);
          if (isApproved == 1)
  ...
  private getReportData(miscellaneousItemId) {
    debugger
    this.apiUrl = "";
    let userInfo = this.commonService.GetUserProfileJson();
    this.apiUrl = `SalesInvoiceReport/GetMiscellaneousReportDepotById?reportFormat=Pdf&userId=${userInfo[0].employeeid}&miscellaneousItemId=${miscellaneousItemId}`;
    console.log('apiUrl: ', this.apiUrl);
    this.commonService.GetCrystalReportData(this.apiUrl).subscribe((returns: any) => {
      let res = JSON.parse(returns);
      if (res.status) {
