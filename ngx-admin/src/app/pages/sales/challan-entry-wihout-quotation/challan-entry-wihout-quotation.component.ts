...
@Component({
  selector: 'ngx-challan-entry-wihout-quotation',
  templateUrl: './challan-entry-wihout-quotation.component.html',
  styleUrls: ['./challan-entry-wihout-quotation.component.scss']
})
export class ChallanEntryWihoutQuotationComponent implements OnInit {
  ...
  public pageNavigation = "Challan without quotation";
...
master: {
  ...
  private save() {
    ...
    if (!this.master.lstDetailsViewModel || this.master.lstDetailsViewModel.length == 0) {
      this.toastrService.danger("Please add a product!", "Message");
      this.commonService.valueSet("create");
      return false;
    }
  ...
  public getProductSpecDetails() {
    ...
    this.master.challanQty = 1;
    this.CalculateTotalPrice();
  }

  public CalculateTotalPrice() {
    const qty = this.master.challanQty || 0;
  ...
  public addToDetailsGrid() {
    ...
    if (!this.master.challanQty || this.master.challanQty <= 0) {
...
public getGridData() {
    ...
    if (this.commonService.validateDates(fromDate, toDate)) {
      this.salesinvoiceService.GetTenderChallanWihoutQuotationById(0, this.commonService.DateFormat(this.master.loadFromDateShow), this.commonService.DateFormat(this.master.loadToDateShow)).subscribe((data: any) => {
        ////debugger;
        if (data.success) {
