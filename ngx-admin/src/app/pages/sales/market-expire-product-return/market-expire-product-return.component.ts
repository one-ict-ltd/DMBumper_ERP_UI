...
@Component({
  selector: 'ngx-market-expire-product-return',
  templateUrl: './market-expire-product-return.component.html',
  styleUrls: ['./market-expire-product-return.component.scss']
})
export class MarketExpireProductReturnComponent implements OnInit {
  ...
  constructor(
    ...
    this.columnDefs = [
      ...
      }, /// Dont Change
      {
        ...
        field: "damageExpireProductReturnNo",
        width: 220,
      },
      {
        headerName: "Date",
        field: "damageExpireProductReturnDate",
        width: 160,
      },
      {
        headerName: "Type",
  ...
  public pageNavigation = "Market Expire Products Transfer To Factory";//"Damage Expire Products Return";
  ...
  private save() {
    ...
    if (this.checkCount == 0) {
      this.commonService.valueSet("create");
      this.toastrService.danger("Please Select at least one product", "Message");
      return;
    }
    this.master.startDate = this.commonService.DateFormat(this.master.startDate);
    var button = this.commonService.buttonClicked;
    this.salesinvoiceService
      .SetDamageExpireProductsReturn(this.master)
  ...
  private agReport(event) {
    //alert(event.data.damageExpireProductReturnMasterId);
    this.getReportData(event.data.damageExpireProductReturnMasterId);
    // this.generateReport1(event.data.dispatchMasterId, "print");
    //this.GetReport(event.data.dispatchMasterId);
  }
  private getReportData(masterId) {
    debugger
    let userInfo = this.commonService.GetUserProfileJson();
    this.apiUrl = `SalesInvoiceReport/GetDamageOrExpireReportById?reportFormat=Pdf&userId=${userInfo[0].employeeid}&masterId=${masterId}&MarketOrDepo=Market`;
    debugger;
  ...
  loadGridData() {
    this.salesinvoiceService
      .GetAllDamageExpireProductReturn(this.master.MarketOrDepo).subscribe((data: any) => {
        if (data.success) {
  ...
  ToggleChange(event: any) {
    ...
    this.master.lstMasterViewModel.forEach(element => {
      ...
      if (isChecked) this.checkCount++;
    });
  }
  checkQty(rowIndex: any) {
    debugger
    if (this.master.lstMasterViewModel[rowIndex].qty > this.master.lstMasterViewModel[rowIndex].pendingQty) {
      this.master.lstMasterViewModel[rowIndex].qty = this.master.lstMasterViewModel[rowIndex].pendingQty;
      this.toastrService.warning("Message", "Transfer Qty can not be greater that pending Qty");
    }

  }
  LoadAllDropdown() {
    //this.getEmployee();
    this.GetProductExpireReturnItemMarketListJson();
    this.loadApprovalStatusList();
  }

  GetProductExpireReturnItemMarketListJson() {
    // this.commonService.valueSet("create");
    // this.master.typeId = this.master.typeSelected['id'];
    // this.salesinvoiceService
    //   .GetProductExpireReturnItemMarketListJson()
    //   .subscribe((returns: any) => {
    //     if (returns.success) {
  ...
  GetSalesInvoiceMasterListForApproval() {
    this.salesinvoiceService
      .GetMiscellaneousItemMarketListJson()
      .subscribe((returns: any) => {
        if (returns.success) {
          this.master.lstMasterViewModel = returns.data;
          //console.log(this.master.lstMasterViewModel);
  ...
  //   this.gttlAmount = 0;
  //   this.bodyData = [];
  //   var MarketOrDepo = 'Market';
  //   debugger;
  //   this.apiUrl = `SalesInvoice/GetAllDamageExpireReturnByIdJson?masterId=${collectionId}&MarketOrDepo=${MarketOrDepo}`;
  //   this.commonService.getReportData(this.apiUrl).subscribe((returns: any) => {
  //     if (returns.success) {
  //       this.datalength = returns.data.length * 50;

  //       returns.data.forEach(element => {
  //         this.gttlAmount += element.ttlAmount ?? 0;
  //       });

  //       this.bodyData = returns.data;
  //       this.dispatchNo = returns.data[0].damageExpireProductReturnNo;
  //       this.dispatchDate = returns.data[0].damageExpireProductReturnDate;
  //       this.dispatcherName = returns.data[0].miscellaneousType;
  ...
  checkChange() {
    ...
    this.master.lstMasterViewModel.forEach(el => {
      if (el.isSelect) this.checkCount++;
    });
  }
}
