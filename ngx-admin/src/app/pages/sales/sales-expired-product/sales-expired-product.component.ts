...
@Component({
  selector: 'ngx-sales-expired-product',
  templateUrl: './sales-expired-product.component.html',
  styleUrls: ['./sales-expired-product.component.scss']
})
export class SalesExpiredProductComponent implements OnInit {
  ...
  public pageNavigation = "Expired Product Return";
  ...
  addToGrid() {
    ...
    else if (this.invoiceSelected == undefined || this.invoiceSelected == null) {
      this.toastrService.danger("Please select a Invoice.", "Message");
      return;
    }
    ...
    else if (this.returnQty == 0) {
      this.toastrService.danger("Please input Exp. Return Qty.", "Message");
      return;
    }
    else if (this.amount == 0) {
      this.toastrService.danger("Please input Exp. Return Amount.", "Message");
      return;
    }
    else if (this.amount > this.dueAmount) {
    ...
    this.master.lstDetailsViewModel.forEach(x => {
      debugger;
      if (x.salesInvoiceId == this.invoiceSelected['id'])
        Total += x.amount
    });

    if (this.dueAmount >= Total) {
      ...
      //this.master.lstDetailsViewModel.push(item);
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      //this.getStore();
      this.salesreturnService
        .GetSalesPExpireReturnById(salesReturnMasterId)
        .subscribe((data: any) => {
          if (data.success) {
  ...
  private agDelete(event) {
    if (confirm(`Are you sure to delete this ( ${event.node.data.expireReturnNumber} ) number?`)) {
      ...
      this.salesreturnService
        .SalSpDeleteSalesPExpireReturn(salesReturnMasterId)
        .subscribe((returns: any) => {
          if (returns.success) {
  ...
  public generateReport(salesReturnMasterId) {
    this.salesreturnService
      .GetSalesPExpireReturnDetailsByMasterId(salesReturnMasterId)
      .subscribe((returns: any) => {
        if (returns.success) {
          ...
          // this.rgrandTotal = this.bodyData[0]["amount"];
