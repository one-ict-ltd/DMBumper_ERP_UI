...
import { ProductcategoryService } from "app/pages/inventory/settings/productcategory.service";
import { CommoncomboService } from "app/services/commoncombo.service";
import { CommonButtonComponent } from "app/pages/common/common-button/common-button.component";
...
@Component({
  selector: 'ngx-approvertype',
  templateUrl: './approvertype.component.html',
  styleUrls: ['./approvertype.component.scss']
})
export class ApproverTypeComponent implements OnInit {
  ...
  public pageNavigation = "Approver Type";
  ...
  private save() {
    ...
    debugger
    if (this.master.approvalTypeSelected == null) {
      this.toastrService.danger("Please select Approval Type.", "Message");
      this.commonService.valueSet("create");
      return false;
    }

    if(this.master.employeeSelected == null){
       this.master.employeeId = null
    }

    
    this.show = true;
    this.salesapprovalmatrixService.SaveApproverType(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
           ...
           //////////////Grid Refresh ///////////////////
            this.salesapprovalmatrixService.GetApproverTypeById(0,0).subscribe((data: any) => {
              if (data.success) {
  ...
  constructor(
    ...
    //  private commonbuttonComponent:CommonButtonComponent
  ) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.selectedRow = event.node.data;
      var approverTypeId = event.node.data.approverTypeId;
      this.master.lstDetail = [];
      this.salesapprovalmatrixService.GetApproverTypeById(approverTypeId,0).subscribe((data: any) => {
        if (data.success) {
          debugger;
          ...
          if(this.master.employeeId!=0){
            this.master.employeeSelected = {
              id: data.data[0].employeeId,
              name: data.data[0].approverName,
            }
           
          }
          //  else{
          //   this.master.employeeId = 0
          // }
  ...
  private agDelete(event) {
    debugger
    this.master.approverTypeId = event.node.data.approverTypeId;
    this.salesapprovalmatrixService.DeleteApproverTypeById(this.master.approverTypeId).subscribe((returns: any) => {
      if (returns.success) {
