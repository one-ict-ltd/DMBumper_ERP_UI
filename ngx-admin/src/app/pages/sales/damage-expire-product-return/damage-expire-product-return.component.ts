...
@Component({
  selector: 'ngx-damage-expire-product-return',
  templateUrl: './damage-expire-product-return.component.html',
  styleUrls: ['./damage-expire-product-return.component.scss']
})
export class DamageExpireProductReturnComponent implements OnInit {
  ...
  public pageNavigation = "Damage, Expire, Re-Work Products Transfer To Factory";//"Damage Expire Products Return";
  ...
  GetSalesInvoiceMasterListForApproval() {
    ...
    this.salesinvoiceService
      .GetMiscellaneousItemDepotListJson(this.master.typeId)
      .subscribe((returns: any) => {
        if (returns.success) {
  ...
  private getReportData(masterId) {
    debugger
    let userInfo = this.commonService.GetUserProfileJson();
    this.apiUrl = `SalesInvoiceReport/GetDamageOrExpireReportById?reportFormat=Pdf&userId=${userInfo[0].employeeid}&masterId=${masterId}&MarketOrDepo=Depo`;
    debugger;
    this.commonService.GetCrystalReportData(this.apiUrl).subscribe((returns: any) => {
      let res = JSON.parse(returns);
      if (res.status) {
