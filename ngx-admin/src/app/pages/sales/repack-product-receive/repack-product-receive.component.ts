...
@Component({
  selector: 'ngx-repack-product-receive',
  templateUrl: './repack-product-receive.component.html',
  styleUrls: ['./repack-product-receive.component.scss']
})

export class RepackProductReceiveComponent implements OnInit {

  pageNavigation = "Repack Product Receive(Depot)";
  ...
  public getMaster() {
    this.master = {
      ...
      mgfDate: new Date(),
      expireDate: new Date(),
      RePackProductTransferSelected: null,
      RePackProductTransferId: 0,
      RePackProductTransferNo: null
    };
  ...
  public GetRePackProductTransferNoListForReceive() {
    debugger;
    //this.ProducttransferService.GetRePackProductTransferById(0).subscribe(
    this.ProducttransferService.GetRePackProductTransferNoListForReceive(0).subscribe(
      (data: any) => {
        this.RePackProductTransferList = data.data.map((val) => ({
          id: val.RePackProductTransferId,
          name: val.RePackProductTransferNo,
        }));
      }
    );
  }
    ...
    this.miscellaneousItemService
      .GetMiscellaneousItemDepotById().pipe(first()).subscribe(
        (data: any) => {
          debugger;
          if (data.success) {
            this.rowData = data.data.filter((item: any) => item.miscellaneousTypeId === 14);
          } else {
            this.rowData = [];
          }
        },
  ...
  LoadAllDropdown() {
    ...
    this.GetRePackProductTransferNoListForReceive();
    //this.getProductList();
    this.getTypeList();
  }

  public getRePackTransferDetails(event) {
    debugger
    this.master.RePackProductTransferNo = event.name;
    this.master.RePackProductTransferId = event.id;

    this.ProducttransferService.GetRePackTransferDetailsById(
      event.id
    ).subscribe((returns: any) => {
      if (returns.success) {
        debugger;
        this.master.lstMiscellaneousDetailsViewModel = returns.data;

      } else this.master.lstMiscellaneousDetailsViewModel = [];
    });
  }
 ...
 */
    this.typeList = [{ id: 14, name: 'Re-Pack / Re-Work (Receive)' },];
    // this.typeList = [];
