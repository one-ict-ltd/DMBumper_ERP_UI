...
export class SalesinvoiceComponent implements OnInit {
  ...
  constructor(
    ...
    this.columnDefs = [
      ...
      //     commonService.currencyFormatter(params.data.totalGross),
      //   type: "rightAligned",
      // },
      // {
  ...
  GetGridData() {
    this.salesinvoiceService.GetSalesInvoiceById(0, this.commonService.DateFormat(this.loadFromDateShow), this.commonService.DateFormat(this.loadToDateShow)).subscribe((data: any) => {
      if (data.success) {
        this.rowData = data.data;
      }
    });
  ...
  GetCollectionDiscountNotApplicableProductList() {
    ...
    this.salesinvoiceService.GetCollectionDiscountNotApplicableProductList(0).subscribe((data: any) => {
      if (data.success) {
        ...
        console.log('DiscountNotApplicableProductList=', this.CollectionDiscountNotApplicableProductList);
      }
    });
  }
  ...
  public onRowClicked(event) {
    ...
    if (data == "edit") {
      if (event.node.data.isClosed == 1) {
        this.toastrService.warning(`This Invoice is already GDN confirmed, so you can not edit this invoice.`, 'Warning !')
        return;
      } else {
        this.agEdit(event);
        this.show = false;
    ...
    if (temp === 0) {
      ...
      this.salesinvoiceService
        ...
        .subscribe((data: any) => {
          if (data.success) {
            debugger;
            let tCode = data.data[0].TerritoryCode;
            this.master.territoryCode = data.data[0].TerritoryCode;
            this.GetAllActivePartyByTypeId(0);

            this.master = data.data[0];
            this.master.collectionDate = (data.data[0].collectionDate != null || data.data[0].collectionDate != '') ? new Date(data.data[0].collectionDate) : null;


            this.master.territoryCode = tCode;

            this.master.terriSelected = {
            ...
            let transactionTypeId = data.data[0].transactionTypeId ?? 0;
            this.master.transactionTypeSelected = {
              id: ((transactionTypeId) = 0 ? 2 : transactionTypeId),
              name: (data.data[0].transactionTypeName == null ? "" : data.data[0].transactionTypeName),
            };
            ...
            this.master.partySelected = {
              ...
              territoryDetails: (data.data[0].territoryDetails == undefined || null) ? '' : data.data[0].territoryDetails,
            };
            this.master.vat = 0;
            this.master.ait = 0;
    ...
    this.salesinvoiceService
      ...
      .subscribe((returns: any) => {
        if (returns.success) {
          ...
          this.salesinvoiceService
            .GetSalesInvoiceById(0, this.commonService.DateFormat(this.loadFromDateShow), this.commonService.DateFormat(this.loadToDateShow))
            .subscribe((data: any) => {
              if (data.success) {
                this.rowData = data.data;
              }
  ...
  public pageNavigation = "Sales Invoice";
    ...
    if (this.commonService.buttonClicked == "create") {
      ...
      if (this.StoreList.length > 0) {
        ...
        if (this.transactionTypeList.length > 0) {
          ...
          this.master.transactionTypeSelected = { id: this.transactionTypeList[0].id, name: this.transactionTypeList[0].name };
        }
      ...
      this.show = false;
      // if (this.StoreList.length > 0)
      //   this.master.storeId = this.StoreList[0].storeId;
      //alert(this.master.storeId);
  ...
  getServerDateTime() {
    ...
    this.commonService.getApiData(apiUrl).subscribe((returns: any) => {
      if (returns.success) {
        ...
        if (this._CompanyId == 1) this.isReadonly = returns.data[0].siPriceReadonly_HH;
        if (this._CompanyId == 2 || this._CompanyId == 3) this.isReadonly = returns.data[0].siPriceReadonly_AH;
...
GetAreaByDepoCode(DepoCode);
  */
  ...
  private save() {
    debugger;
    let gid: string = this.commonService.getUserGroup() ?? "0";
    var button = this.commonService.buttonClicked;

    this.commonService.valueSet("create");

    let ttlDiscountAmnt: number = 0;
    let hasNationalBonusTtlCount: number = 0;
    ...
    for (let index = 0; index < this.master.lstDetailsViewModel.length; index++) {
      ...
      if ((element.hasNationalBonus ?? 0) == 1) {
        hasNationalBonusTtlCount += element.hasNationalBonus;
        ttlDiscountAmnt += element.discountAmount;
        ttlNationalAmnt += element.price * element.invoiceQty
      }
      //added on: 2025-Jan-09
    ...
    if (
      this.master.lstDetailsViewModel == null || this.master.lstDetailsViewModel.length == 0) {
      this.toastrService.warning("Please entry product.", "Warning");
      return false;
    }
      ...
      .getTerritoryOfficerByPartyId(this.master.partyId).subscribe((returns: any) => {
        if (returns.success) {
          if (returns.data[0].employeeId == 0) {
            ...
            this.master.paymentDate = this.master.paymentDate == null ? null : this.commonService.DateFormat(this.master.paymentDate);
            ...
            this.salesinvoiceService
              ...
              .subscribe((returns: any) => {
                ...
                if (returns.success) {
                  ...
                  let tCode = this.master.territoryCode;
                  let tSelected = this.master.terriSelected;

                  this.getMaster();

                  this.master.territoryCode = tCode;
                  this.master.terriSelected = tSelected; //{ id: tSelected["id"], name: tSelected["name"] };
                  //this.GetGridData();
                  if (this.transactionTypeList.length > 0) {
                    this.master.transactionTypeId = this.transactionTypeList[0].id;
                    this.master.transactionTypeSelected = { id: this.transactionTypeList[0].id, name: this.transactionTypeList[0].name };
                  ...
                  // console.log("this.master.storeId ", this.master.storeId);
                  // console.log("this.master.transactionTypeId  ", this.master.transactionTypeId);
 ...
 */
    ...
    if (this.master.productSpecSelected == undefined || this.master.productSpecSelected == null) return;
    ...
    this.commonService.getApiData(apiUrl).subscribe((returns: any) => {
      ...
      if (returns.success) {
        ...
        if (returns.data.length > 0) {
          ...
          this.master.currentStock = returns.data[0].totalCurrentStock;
          this.master.BatchSelected = {
  ...
  public GetPartyDetails() {
    ...
    this.resetCtrl();
    //
    this.salesinvoiceService.GetCustomerDuesStatus(this.master.partyId, this.master.territoryCode).subscribe((returns: any) => {
      if (returns.success) {
        this.TerritoryDetails = `${this.master.partySelected["territoryDetails"]}; Has Deed: ${returns.data[0].hasDeed}; Credit Limit: ${returns.data[0].creditLimit} TK; Dues Amount: ${returns.data[0].duesAmount} TK; Credit Limit Crossed: ${returns.data[0].creditLimitCrossed}; Has Over Dues Days: ${returns.data[0].overDuesStatus}; Has Contact No: ${returns.data[0].hasMobileNo}; Has Address: ${returns.data[0].hasAddress}`;

        this.balanceAmount = returns.data[0].creditLimit - returns.data[0].duesAmount;

        if (returns.data[0].hasDeed == "No") {
          this.hasDeed = false;
          if (this.master.salesInvoiceId == 0) this.master.lstDetailsViewModel = [];
          this.master.totalGross = 0;
          this.master.grandTotal = 0;
          //this.toastrService.warning("This customer has no deed.", "Warning !!!")
        }
        ...
        else if (returns.data[0].hasMobileNo == "No") {
          this.hasMobileNo = false;
          if (this.master.salesInvoiceId == 0) this.master.lstDetailsViewModel = [];
          this.master.totalGross = 0;
          this.master.grandTotal = 0;
          this.toastrService.warning("This customer has no contact number.", "Warning !!!")
        }
        else if (returns.data[0].hasAddress == "No") {
          this.hasAddress = false;
          if (this.master.salesInvoiceId == 0) this.master.lstDetailsViewModel = [];
          this.master.totalGross = 0;
          this.master.grandTotal = 0;
          this.toastrService.warning("This customer has no address.", "Warning !!!")
        }
        else {
          this.hasDeed = true;
    ...
    this.salesinvoiceService.GetCreditNoteList(this.master.partyId, this.master.salesInvoiceId).subscribe((returns: any) => {
      if (returns.success) {
        ...
        if (this.master.lstCreditNoteViewModel.length > 0) {
          this.Credit = true;
          this.creditNoteMsg = "This Customer Has pending Credit Note for adjustment. Please see the details end of this page ";
          this.toastrService.info("This Customer Has pending Credit Note for adjustment. Please see the details end of this page ", "");
        }
        else {
          this.creditNoteMsg = "";
          this.Credit = false;
  ...
  checkChange(rowIndex: number) {
    ...
    this.AdjustAmount = 0;
    for (let index = 0; index < this.master.lstCreditNoteViewModel.length; index++) {
      const el = this.master.lstCreditNoteViewModel[index];
      // }
  ...
  public async calculateTotal(index: number) { // in list

    let totalPrice = 0;
    debugger;
    let invoiceQty =
      this.master.lstDetailsViewModel[index].invoiceQty; //== (null || 0) ? 1 : this.master.lstDetailsViewModel[index].invoiceQty;

    invoiceQty = (invoiceQty ?? 0) ? 1 : invoiceQty;

    this.master.lstDetailsViewModel[index].invoiceQty = invoiceQty;

    let currentStock = this.master.lstDetailsViewModel[index].currentStock ?? 0;


    let price =
      (this.master.lstDetailsViewModel[index].price == null || this.master.lstDetailsViewModel[index].price == "")
        ? 0
        : this.master.lstDetailsViewModel[index].price;

    let vat =
      (this.master.lstDetailsViewModel[index].vat == null || this.master.lstDetailsViewModel[index].vat == "")
        ? 0
        : this.master.lstDetailsViewModel[index].vat;

    let ait =
      (this.master.lstDetailsViewModel[index].ait == null || this.master.lstDetailsViewModel[index].ait == "")
        ? 0
    ...
    if (this._CompanyId == 1 || this._CompanyId == 2 || this._CompanyId == 3) {
      ...
      this.isDisabled = true;
      const returns: any = await this.salesinvoiceService
        .GetItemWsieBonus(
      ...
      if (returns.success) {
        ...
        */
        ...
        let discountAmount2 = (this.master.lstDetailsViewModel[index].discountAmount == null || this.master.lstDetailsViewModel[index].discountAmount == "") ? 0 : this.master.lstDetailsViewModel[index].discountAmount;
      ...
      let discountAmount =
        (this.master.lstDetailsViewModel[index].discountAmount == null || this.master.lstDetailsViewModel[index].discountAmount == "")
          ? 0
  ...
  calculateGrandTotal() {
    let totalGross = 0;
    for (let index = 0; index < this.master.lstDetailsViewModel.length; index++) {
      const el = this.master.lstDetailsViewModel[index];
      //this.master.lstDetailsViewModel.forEach((row) => {
      totalGross += el.total == "" ? 0 : el.total;
    };
    //);
  ...
  public addToDetailsGrid(e: any) {
    ...
    /*
        ...
        const prod = this.CollectionDiscountNotApplicableProductList.filter(x => x.isFlatRate == 1 && x.productWiseSpecificationId == this.master.productWiseSpecificationId)
        console.log("FilteredDiscountNotApplicableProductList", prod);
        ...
        if (count > 0) {
          ...
          debugger;
          if (hasCollDiscount != avg && this._CompanyId != 2) {
            this.toastrService.danger('Please create seperate invoice for "Flat Rate" Product.', 'Warning !');
            return;
          }
          if (this._CompanyId == 2) {
            if (hasCollDiscount != avg && this.discountType.trim() == '( Flat discount applied )') {
              this.toastrService.danger('Please create seperate invoice for "Flat Rate" Product.', 'Warning !');
              return;
            }
          }
    ...
    */
    ...
    // if(this.batchs.count()>1&& this.BatchList[0].)
    let elements = {
      salesInvDetailsId: 0,
      ...
      currentStock: this.master.currentStock ?? 0,
  ...
  getTotal() {
    ...
    debugger;
    //tp = tp - (this.master.tdiscount == null ? 0 : this.master.tdiscount * this.master.invoiceQty);
    tp = tp - (this.master.discountAmount == null ? 0 : this.master.discountAmount * this.master.invoiceQty);

    let tvat = this.master.vat == null ? 0 : (this.master.vat * this.master.invoiceQty);
    let tait = this.master.ait == null ? 0 : (this.master.ait * this.master.invoiceQty);
    debugger;
    this.master.tTotal = this.roundToDigit(((tp + tvat + tait)), 2);
  }
  resetCtrl() {
  ...
  public async GetItemWsieBonus() {
    ...
    console.log('hasNationalBonus: ', this.hasNationalBonus);
    debugger;
    //this.master.currentStock = 0;
    this.master.tdiscount = this.master.discountAmount;
    this.master.discountAmount = 0;
    this.discountType = '';
    ...
    this.isDisabled = true;
    if (this.master.productWiseSpecificationId > 0) {
      const returns: any = await this.salesinvoiceService.GetItemWsieBonus(
        this.commonService.DateFormat(this.master.salesInvoiceDate),
        this.master.partyId,
        this.master.productWiseSpecificationId,
      ...
      if (returns && returns.success) {
        ...
        let discountAmount = returns.data[0].discountAmount;
        let requestedInvoiceQty = returns.data[0].requestedInvoiceQty;

        //this.master.discountAmount = this.calculateDiscount(this.master.price);

        if ((this._CompanyId == 1 || this._CompanyId == 2 || this._CompanyId == 3) && requestedInvoiceQty == (this.master.invoiceQty ?? 0)) {
          this.master.discountAmount = discountAmount;
          ...
          //this.isDisabled = false;// Added on 2025-01-09
        }
        else { this.CalculateTotalPrice(); }


        this.isDisabled = false; // 2025-01-09
      }
      else {
        this.hasNationalBonus = null;
        this.toastrService.danger('Network error occurred', 'Warning !');
      ...
      this.salesinvoiceService
        ...
        .subscribe((returns: any) => {
          ...
          if (returns.success) {
            ...
            // else if (this._CompanyId == 2) {

            //   //this.master.discountAmount = discountAmount;
            //   if (price > 0) {
            //     this.master.price = price;
  ...
  VerifyNationalBonusForGrid() {
    ...
    for (let index = 0; index < this.master.lstDetailsViewModel.length; index++) {
      ...
      if (element.hasNationalBonus == 1) {
        ttlNationalAmnt += element.price * element.invoiceQty //element.total;
      }
    };
    //);
    ...
    */
    ...
    for (let index = 0; index < this.master.lstDetailsViewModel.length; index++) {
      ...
      //this.calculateTotal(index); // don't call
  ...
  async VerifyBonusDiscountGridBeforeSave() {
    console.log(this.master.lstDetailsViewModel);
    debugger;
    ...
    for (let index = 0; index < this.master.lstDetailsViewModel.length; index++) {
      ...
      if (this.master.salesInvoiceDate == null || this.master.productWiseSpecificationId == 0 || this.master.partyId == 0) return false;
      ...
      if (this.master.productWiseSpecificationId > 0) {
        ...
        if (returns.success) {
          ...
          if (price > 0 && this.hasNationalBonus == 0) {
            //el.price = price;
            el.discountAmount = discountAmount;
            el.hasNationalBonus = this.hasNationalBonus;
          }
          ...
          //this.CalculateTotalPrice();// Need to Set row total and discount
    ...
    for (let index = 0; index < this.master.lstDetailsViewModel.length; index++) {
      const element = this.master.lstDetailsViewModel[index];
      if (element.hasNationalBonus == 1) ttlNationalAmnt += element.price * element.invoiceQty
    };
    for (let index = 0; index < this.master.lstDetailsViewModel.length; index++) {
      const element = this.master.lstDetailsViewModel[index];
  ...
  public deleteDetails(index: any) {
    debugger;
    let gid: string = this.commonService.getUserGroup() ?? "0"; //1 for SuperAdmin

    if (gid != "1" && this.master.salesInvoiceId > 0 && (this.master.hasCollection > 0 || this.master.hasPicking > 0)) {
      if (this.master.collectionDate != null && (this.master.collectionDate < this.master.salesInvoiceDate)) {
