...
@Component({
  selector: 'ngx-customer-obserbation-entry',
  templateUrl: './customer-obserbation-entry.component.html',
  styleUrls: ['./customer-obserbation-entry.component.scss']
})
export class CustomerObserbationEntryComponent implements OnInit {
...
GetAreaByDepoCode(DepoCode);
  */
  ...
  private save() {
    ...
    this.partyService.setpartyObservation(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
        ...
        this.show = true;
        //////////////Grid Refresh ///////////////////
        this.partyService.AccSpGetPartyObserbationGridJson().subscribe((data: any) => {
          if (data.success) {
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.partyService.getpartyObserbation(partyId).subscribe((data: any) => {
        if (data.success) {
          debugger;
