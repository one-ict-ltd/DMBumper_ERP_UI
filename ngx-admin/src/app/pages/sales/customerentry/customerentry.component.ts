...
@Component({
  selector: 'ngx-customerentry',
  templateUrl: './customerentry.component.html',
  styleUrls: ['./customerentry.component.scss']
})
export class CustomerentryComponent implements OnInit {
  ...
  public pageNavigation = "Customer Entry";
  ...
  public getTerritoryDetails() {
    this.TerritoryDetails = "";
    this.TerritoryDetails = this.master.territorySelected["ZoneName"] + " -> " + this.master.territorySelected["RegionName"] + " -> " + this.master.territorySelected["AreaName"] + " -> " + this.master.territorySelected["name"];

    if (this.master.territorySelected["asmCode"] == 'N/A') {
      this.master.isApproved = 2
    } else {
      this.master.isApproved = 1
    }
...
GetAreaByDepoCode(DepoCode);
  */
  ...
  public getDuplicatePartyCode() {
    this.partyService.getDuplicatePartyCode(this.master.partyId, this.master.partyCode).subscribe((returns: any) => {
      this.partyCodeIsDuplicate = returns.data[0].countData > 0 ? true : false;
    });
  }


  public getNewPartyCode() {
    this.partyService.getNewPartyCode().subscribe((returns: any) => {
      if (returns.status) {
        this.master.partyCode = returns.data[0]?.partyCode;
      }
    });
  }
  ...
  private save() {
    ...
    else if (this.master.partyCode == null || this.master.partyCode.trim() == "") {
      this.toastrService.danger("Please input party code", "Message");
      this.commonService.valueSet("create");
      return false;
    }
    else if (this.partyCodeIsDuplicate) {
      this.toastrService.danger("Duplicate Party Code", "Message");
      this.commonService.valueSet("create");
      return false;
    }
    ...
    else if (this.master.territorySelected["id"] == null) {
    ...
    if (!this.commonService.isValidBdMobileLocal(this.master.contactNumber)) {
      this.toastrService.danger("Please insert a valid contact number", "Message");
      this.commonService.valueSet("create");
      return false;
    }
  ...
  GetGridData() {
    if (this.master.territoryCode) {
      this.master.terriSelected = this.territoryList.find(x => x.id == this.master.territoryCode);
    }

    this.partyService.GetPartyByIdJsonNewNewForGrid(this.master.territoryCode).subscribe((data: any) => {
      //debugger;
      console.log("party count", data.data.length)
      if (data.success) {
        this.rowData = data.data;
        console.log("party data bind", data.data.length)
      }
    });
  }
  ...
  onGridReady(params) {
    ...
    // this.partyService.GetPartyByIdJsonNewForGrid().subscribe((data: any) => {
    //   //debugger;
    //   console.log("party count", data.data.length)
    //   if (data.success) {
    //     this.rowData = data.data;
