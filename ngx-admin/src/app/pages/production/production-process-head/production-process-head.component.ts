...
@Component({
  selector: 'ngx-production-process-head',
  templateUrl: './production-process-head.component.html',
  styleUrls: ['./production-process-head.component.scss']
})
export class ProductionProcessHeadComponent implements OnInit {
  ...
  public pageNavigation = "Production Process Head";
  ...
  private save() {
    if (this.master.headName == "") {
      ...
      this.productionProcessService.SaveProductionProcessHead(this.master).subscribe((returns: any) => {
        if (returns.success) {
          if (button == "update") {
  ...
  private agDelete(event) {
    debugger;
    this.master.processHeadId = event.node.data.processHeadId;
    ...
    if (confirm('Are you sure?')) {
      this.productionProcessService.DeleteProductionProcessHeadById(this.master.processHeadId).subscribe((returns: any) => {
        if (returns.success) {
          this.toastrService.success(this.commonService.deletedmsg, "Message");
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.productionProcessService.GetProductionProcessHeadById(this.master.processHeadId).subscribe((data: any) => {
        if (data.success) {
          this.master = data.data[0];
        }
      })
