...
@Component({
  selector: 'ngx-productionplan-report-from-date-range',
  templateUrl: './productionplan-report-from-date-range.component.html',
  styleUrls: ['./productionplan-report-from-date-range.component.scss']
})

export class ProductionplanReportFromDateRangeComponent implements OnInit {
  ...
  pageNavigation = "Production Plan with Date Range";
  ...
  generateCrReport(reportFormat: any) {
    ...
    this.apiUrl = `ProductionReport/GetProductionPlanReportByIdFromDateRange?reportFormat=${reportFormat}&userId=${userInfo[0].employeeid}&productionPlanId=0&fromDate=${this.commonService.DateFormat(this.fDate)}&toDate=${this.commonService.DateFormat(this.tDate)}&productWiseSpecificationId=${productWiseSpecificationId}&batchNo=${this.batchNo}`;
  ...
  private GetPreviewData() {
    ...
    this.apiUrl = `ProductionReport/GetProductionPlanReportByIdFromDateRange?reportFormat=Pdf&userId=${userInfo[0].employeeid}&productionPlanId=0&fromDate=${this.commonService.DateFormat(this.fDate)}&toDate=${this.commonService.DateFormat(this.tDate)}&productWiseSpecificationId=${productWiseSpecificationId}&batchNo=${this.batchNo}`;
