...
@Component({
  selector: 'ngx-requisition-for-rm',
  templateUrl: './requisition-for-rm.component.html',
  styleUrls: ['./requisition-for-rm.component.scss']
})
export class RequisitionForRMComponent implements OnInit {
  ...
  public pageNavigation = "Requisition For Raw Materials (RM)";
  public rptHeader = "RM Requisition";
  ...
  public getAllBatch() {
    debugger
    this.productionPlanService.GetProductionPlanWithType(0, 'raw').subscribe((data: any) => {
      if (data.success) {
  ...
  getBatchWiseBOMDetails(planId: any) {
    ...
    this.master.bomMasterProductWiseSpecificationId = 0;
    this.productionPlanService.GetProductionPlanForRequisitionWithType(planId, 'raw').subscribe((data: any) => {
      if (data.success) {
        ...
        this.bomForSelected = {};
        this.bomService.GetBOMForListFromBOM(planId, 'raw').subscribe((data: any) => {
          if (data.success) {
  ...
  private save() {
    ...
    this.productionServiceService.SaveRMRequisitionMaster(this.master).subscribe((returns: any) => {
      if (returns.success) {
        ...
        //   }
        // });
        // this.productionServiceService.GetRMRequisitionMasterById(0).subscribe((data: any) => {
        //   if (data.success) {
        //     this.rowData = data.data.filter(x => x.type == "raw");
        //   }
        // });
        ...
        if (data.success) {
          ...
          this.productionServiceService.GetRMRequisitionDetialsByMasterId(rmId).subscribe((data: any) => {
            ...
            this.listData.forEach((row) => {
              row.qty = this.roundToDigit(row.qty, 5);
