...
@Component({
  selector: 'ngx-pm-issue',
  templateUrl: './pm-issue.component.html',
  styleUrls: ['./pm-issue.component.scss']
})
export class PmIssueComponent implements OnInit {
  ...
  public pageNavigation = "Issue for PM Requisition";
  ...
  private save() {
    ...
    this.productionServiceService.SaveIssueMaster(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
  ...
  beforeSave() {
    this.listData.forEach((element) => {
      var obj = {
        ...
        requisitinDetailId: element.requisitionDetailId,
  ...
  private agDelete(event) {
    ...
    if (confirm('Are you sure?')) {
      this.productionServiceService.DeleteIssueById(issueId).subscribe((data: any) => {
        if (data.success) {
  ...
  public getRequisitionNo() {
    ...
    this.productionServiceService.GetMaxIssueMasterNumber(this.datePipe.transform(this.master.issueDate, "yyyy-MM-dd"), 2).subscribe((returns: any) => {
      if (returns.success) {
    ...
    debugger;
    this.productionServiceService.GetRequisitionNumberforIssue(2).subscribe((data: any) => {
      if (data.success) {
  ...
  public getTotalQty(rowIndex: any, flag: any) {
    debugger
    let currentstock = this.listData[rowIndex].currentstock ?? 0;
    let IssuedQty = this.listData[rowIndex].qty ?? 0;

    if (IssuedQty > currentstock) {
      this.toastrService.warning("You do not have enough Current Stock for " + this.listData[rowIndex].productName + " issue!", "Warning");
      this.listData[rowIndex].qty = 0;
      this.listData[rowIndex].totalQty = 0;
    }
