...
@Component({
  selector: 'ngx-reagent-receive',
  templateUrl: './reagent-receive.component.html',
  styleUrls: ['./reagent-receive.component.scss']
})
export class ReagentReceiveComponent implements OnInit {
  ...
  public pageNavigation = "Receive for Reagent Requisition";
  ...
  private save() {
    ...
    this._reagentService.SaveReagentReceiveMaster(this.master).subscribe((returns: any) => {
      if (returns.success) {
        if (button == "update") {
        ...
        this._reagentService.getReagentReceiveListByDate(this.commonService.DateFormat(this.loadFromDateShow), this.commonService.DateFormat(this.loadToDateShow)
  ...
  beforeSave() {
    this.listData.forEach((element) => {
      var obj = {
        ...
        issueDetailId: element.reagentIssueDetailId,
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this._reagentService.GetReagentReceiveDetialsByMasterId(receiveId).subscribe((data: any) => {
        if (data.success) {
          ...
          this.issueSelected = {
            id: data.data[0].reagentIssueMasterId,
  ...
  private agDelete(event) {
    ...
    if (confirm('Are you sure?')) {
      this._reagentService.DeleteReagentReceiveById(receiveId).subscribe((data: any) => {
        if (data.success) {
  ...
  public getRequisitionNo() {
    ...
    this._reagentService.GetMaxReagentReceiveNumber(this.datePipe.transform(this.master.issueDate, "yyyy-MM-dd")).subscribe((returns: any) => {
      if (returns.success) {
        debugger;
    ...
    this._reagentService.GetReagentIssueNumberforReceive().subscribe((data: any) => {
      if (data.success) {
  ...
  public getIssueDataById(id) {
    debugger;
    //this.getMaxNo();
    this._reagentService.GetReagentIssueDetailsByMasterIdForReceive(id).subscribe((data: any) => {
      if (data.success) {
        console.log("requisition data:", data.data[0]);
        this.master.reagentIssueMasterId = data.data[0].reagentIssueMasterId;
