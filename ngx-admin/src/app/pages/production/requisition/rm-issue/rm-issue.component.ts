...
@Component({
  selector: 'ngx-rm-issue',
  templateUrl: './rm-issue.component.html',
  styleUrls: ['./rm-issue.component.scss']
})
export class RmIssueComponent implements OnInit {
  ...
  public pageNavigation = "Issue for RM Requisition";
  ...
  public getRequisitionNo() {
    ...
    this.productionServiceService.GetMaxIssueMasterNumber(this.datePipe.transform(this.master.issueDate, "yyyy-MM-dd"), 1).subscribe((returns: any) => {
      if (returns.success) {
    ...
    debugger;
    this.productionServiceService.GetRequisitionNumberforIssue(1).subscribe((data: any) => {
      if (data.success) {
