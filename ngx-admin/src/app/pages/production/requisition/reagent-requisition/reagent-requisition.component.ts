...
@Component({
  selector: 'ngx-reagent-requisition',
  templateUrl: './reagent-requisition.component.html',
  styleUrls: ['./reagent-requisition.component.scss']
})
export class ReagentRequisitionComponent implements OnInit {
  /////////////////////////////
  master: {
    reagentReqId: number;
    reagentReqDetailsId: number;
    reagentReqNo: string;
    reagentReqDate: Date;
  ...
  public pageNavigation = "Reagent Requisition";
  ...
  public getMaster() {
    this.master = {
      reagentReqId: 0,
      reagentReqDetailsId: 0,
      reagentReqNo: "",
      reagentReqDate: new Date(),
      fromWarehouseId: 0,
      toWarehouseId: 0,
      purpose: "",
      isUrgency: 0,
  ...
  private save() {
    ...
    if (this.master.reagentReqNo == "" || this.master.reagentReqNo == null) {
      this.toastrService.danger("Please enter a product.", "Message");
      this.commonService.valueSet("create");
      return false;
    }
    if (this.master.reagentReqDate == null) {
    ...
    this.show = true;
    this.master.reagentReqDate = this.commonService.DateFormat(this.master.reagentReqDate);
    this.productionService
      .saveReagentReq(this.master)
  ...
  getReagentRequisition() {
    this.productionService
      .getReagentRequisition(0)
      .subscribe((data: any) => {
        if (data.success) {
  ...
  constructor(
    ...
    this.columnDefs = [
      ...
      // },
      {
        headerName: "Reagent Req. No",
        field: "reagentReqNo",
        width: 180,
      },
      {
        headerName: "Reagent Req. Date",
        field: "reagentReqDate",
        width: 180,
      },
      {
  ...
  public CalculateTotalPrice(isAvoid: boolean = false) {
    ...
    let toQty: number = !this.toUomSelected ? 0 : this.toUomSelected["toQty"];
    let fromQty: number = !this.toUomSelected ? 0 : this.toUomSelected["fromQty"];
    let convertionFactor: number = (fromQty / toQty);
    this.master.reqQty = isNaN(convertionFactor) ? this.master.CtnQty : this.master.CtnQty + this.master.convertionQty * convertionFactor;
  ...
  private agEdit(event) {
    ...
    if (temp === 0) {
      ...
      this.selectedRow = event.node.data;
      var reagentReqId = event.node.data.reagentReqId;

      this.productionService
        .getReagentRequisition(reagentReqId)
        .subscribe((data: any) => {
          if (data.success) {
            this.master = data.data[0];
            this.master.reagentReqDate = new Date(this.master.reagentReqDate);
            //console.log(data.data);
            //console.log("agEdit Click");
          }
          this.productionService
            .getReagentReqDetailsById(reagentReqId)
            .subscribe((data: any) => {
              if (data.success) {
  ...
  private agDelete(event) {
    this.master.reagentReqId = event.node.data.reagentReqId;
    this.productionService
      .deleteReagentReqById(this.master.reagentReqId)
      .subscribe((returns: any) => {
        if (returns.success) {
  ...
  public getProductReqNo() {
    if (this.master.reagentReqDate == null) {
      ...
      this.master.reagentReqDate = new Date("dd-MM-yyyy");
    }

    this.productionService
      .GetMaxReagentReqMasterNumber(
        this.master.reagentReqDate.toDateString().substring(4, 15)
      )
      .subscribe((returns: any) => {
        console.log(returns);
        if (returns.success) {
          this.master.reagentReqNo = returns.data[0].MaxNo;
        }
      });
  }
  ...
  GetCurrentStock() {
    ...
    this.commonService.getApiData(apiUrl).subscribe((returns: any) => {
      if (returns.success) {
        this.currentStock = returns.data[0]?.currentStock ?? 0;
      }
    });
  }
  ...
  public addDetails() {
    ...
    if (this.productAlreadyExist) {
      this.toastrService.warning(`Reagent (${this.master.productSelected["name"]}) already in the list !`, "Message");
      return;
    }

    //this.getProductDetails();
    let detail = {
      reagentReqDetailsId: 0, //this.master.lstReqDetailsViewModel.reagentReqDetailsId,
  ...
  public tableHeader = ["#", "Reagent Name", "Reagent Specification", "CTN. Qty", "Loose Qty"];
  private agReport(event) {
    this.generateProductRequisitionReport(event.data.reagentReqId);
  }
  public datalength: number;
  public reagentReqNo = "";
  public reagentReqDate = "";
  public bodyData = [];
  public generateProductRequisitionReport(reagentReqId) {
    this.productrequisitionService
      .getProductReqReportById(reagentReqId)
      .subscribe((returns: any) => {
        if (returns.success) {
          this.bodyData = returns.data;
          this.reagentReqNo = this.bodyData[0].reagentReqNo;
          this.reagentReqDate = this.bodyData[0].reagentReqDate;
          this.setParam();
